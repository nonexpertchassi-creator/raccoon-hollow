 ...
 * 훔쳐가는 양은 한 칸의 12%(많아야 8개)로 작게 두고, 잡았을 때 받는
 * 벌금을 크게 준다. "안 보면 손해"가 아니라 "보면 이득"이 되게.
 *
...
export const THIEF = {
  ...
  every: 75,     // 평균 이 간격으로 (아래 wild 때문에 실제로는 들쭉날쭉하다)
  wild: 1,       // 도둑은 시계처럼 오면 안 된다. 완전히 운
  life: 5.5,     // 도망칠 때까지 주는 시간(초). 이 안에 눌러야 잡는다
  take: 0.12,    // 그 칸 재고의 이만큼을 훔쳐 간다
  max: 8,        // 다만 몇 개까지만
  fine: 20,       // 잡으면 훔치려던 물건값 × 이만큼을 벌금으로 받는다
};
