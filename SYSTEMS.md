...
flowchart TD
    ...
    STAR --> COIN["엽전"]
    COIN --> LEVEL["물건 레벨업"]
    COIN --> BENCH["작업대 한 단 더<br/>(만들 줄 아는 물건 +1)"]
    COIN --> OPENSHOP["가게 열기"]
    COIN --> STAFF["일꾼"]
    COIN --> DOG["삽살개"]
    LEVEL --> IPS["초당 수입"]
...
| 재는 법 | `BAL_HOURS=8 BAL_SEED=1 …/balance.gd` — 운 번호 셋으로 가운뎃값 |
...
| 무엇에 기대나 | **손님 성 합계** · 초당 수입 · 다음 가게가 열려 있는지 · 작업대를 상한까지 세우고 전부 만렙까지 |
| **고치면 흔들리는 것** | 승급하면 **레벨이 1로 돌아간다**(벌이가 46%로 떨어졌다 9레벨에 본전). 작업대 상한이 +2단 오르고 값이 ×4가 된다 |
...
| | |
|---|---|
| 규칙이 사는 곳 | `content.js: SHOPS[].items[].cost` · `sim.gd: bench_lv() · bench_cap() · bench_up()` |
| 무엇에 기대나 | **엽전**만. 한 단 오르면 다음 물건을 만들 줄 알게 된다(앞 물건도 계속 만든다) |
| **고치면 흔들리는 것** | 가짓수가 늘면 **주문이 겹칠 일이 줄어** 노는 일꾼이 준다. 단, 손이 모자라면 늘려도 안 만들어진다 |
| ⚠️ 조심 | 단 상한은 **등급이 정한다**(4·6·8). 승급 조건도 "작업대를 상한까지"라서, 상한 식을 바꾸면 승급이 같이 흔들린다 |
| 재는 법 | `balance.gd`의 '마지막으로 새것이 열린 시각' — 작업대가 막히면 여기가 일찍 멈춘다 |

★ 2026-08-29 이전에는 이것이 **'매대 칸'**이었고, 칸 하나를 열려면 **손님이
그 물건을 물어봐야** 했다. 이름과 자물쇠를 둘 다 갈았다 — 매대에서 파는 일은
없었고(파는 곳은 계산대다), 손님이 오는 것은 이야기가 할 일이기 때문이다.

### 젬

| | |
|---|---|
| 나오는 곳 | 의뢰 · 나쁜 놈 잡기(10%) · 품목 만렙 |
...
| | |
|---|---|
| 일꾼 | 가게마다 **한 명**. 둘째부터는 아무 효과가 없다(재서 확인). 진열대 +10칸, 계산 중 생산이 안 멈춘다 |
...
| ⚠️ 조심 | 화면이 sim보다 **늦다.** 팔린 것은 즉시지만 손님은 걸어온다 — 그 시차를 잊으면 "손님이 오기도 전에 일꾼이 움직인다" 같은 것이 생긴다(실제로 두 번) |
...
**운 번호 하나로 판단하지 않는다.** 이 게임은 운 번호만 바꿔도 8시간 매출이
