...
### 일꾼 너구리 A~D (완성 캐릭터) — `godot/art/hero-body/` · 144×144 · **6장 남음**

가게 구분 없는 **일꾼 너구리** 네 무늬 — 같은 몸 윤곽, 다른 털(A~D).
저장본이 너구리마다 무늬를 무작위 배정한다("쌍둥이 말고 랜덤" — 유저).
방향 둘: `-side`(오른쪽 훼이크 측면 — **팔을 옆으로 뻗은 일하는 손**이
보여야 한다. 일·계산·서기 전부, 왼쪽은 코드 뒤집기) · `-back`(뒷모습).
정면은 안 그린다. 완성된 주문은 코드가 그린 **나무 상자**로 나른다 —
물건 든 그림을 따로 그리지 않는다. **꼬리까지 몸에 그려
넣는다**(흔들기 없음). 발끝은 아래 변, 몸은 가로 정중앙, 그림자 금지.
a 두 장은 이미 들어왔다 — 같은 그림체(통통 너구리)로 나머지를.

- [ ] `b-side.png` — 무늬 B — 옆(일·계산·서기 전부)
- [ ] `b-back.png` — 무늬 B — 뒷모습(위로 걸을 때)
- [ ] `c-side.png` — 무늬 C — 옆(일·계산·서기 전부)
- [ ] `c-back.png` — 무늬 C — 뒷모습(위로 걸을 때)
- [ ] `d-side.png` — 무늬 D — 옆(일·계산·서기 전부)
