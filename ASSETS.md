# 그림 주문서

GPT/그림 도구에 통째로 넘기는 문서. **기획과 그림 사이의 계약서**다.
2026-08-21 기준 — Godot으로 옮기면서 **폴더가 바뀌었다**(아래 첫 칸).

## 어디에 넣나 — `godot/art/` 안이다

```
godot/art/hero/raccoon-make.png     ← 점장 너구리
godot/art/items/pick.png            ← 물건
godot/art/guests/rabbit.png         ← 손님
godot/art/pests/rat.png             ← 나쁜 놈
```

**저장소 뿌리의 `art/`가 아니다.** Godot은 제 프로젝트 폴더(`godot/`) 밖의
파일을 못 읽는다. 밖에 두면 주문서 목록에서는 사라지는데 화면에는 영영
안 나오는, 제일 나쁜 종류의 어긋남이 된다. 옛 `art/`는 통째로 옮겼다.

**넣은 뒤 한 번만:**

```sh
godot --headless --path godot --import
```

Godot이 그림을 제 방식으로 한 번 읽어 두는 과정이다(에디터를 켜 두었다면
저절로 한다). 이걸 안 하면 파일은 있는데 게임이 못 찾는다.

## 크기 — 표의 숫자대로 주면 된다

| 갈래 | 파일 크기 | 화면에 나오는 크기 |
|---|---|---|
| `hero/` 점장 | **144 × 144** | 72 × 72 |
| `items/` 물건 | **128 × 224** | 64 × 112 |
| `staff/` 직원 | **144 × 144** | 72 × 72 |
| `clerks/` 가게별 점장 (선택) | **144 × 144** | 72 × 72 |
| `guests/` 손님 | **128 × 128** | 64 × 64 |
| `pests/` 나쁜 놈 | **128 × 128** | 64 × 64 |

**파일은 화면 크기의 두 배다.** 요즘 폰은 화면이 촘촘해서(같은 1px 자리에
점을 2~3개 찍는다) 화면 크기 그대로 주면 뿌옇게 보인다. 두 배로 받아서
줄여 쓰면 또렷하다. 게임 안에서 크게·작게(0.45~1.8배)도 하므로 여유가 필요하다.

## 규격 (전부 공통)

- **PNG, 배경 투명.** 흰 배경으로 오면 못 쓴다
- **여백 없이 꽉 채우기.** 그림 주위 빈 공간은 배치를 어긋나게 만든다
- **발끝이 이미지 아래 변에 닿아야 한다** — 코드가 발끝을 그 자리에 놓고,
  발끝 높이로 앞뒤를 가린다. 발끝이 붕 떠 있으면 공중에 서게 된다
- **오른쪽을 보는 것 하나만.** 왼쪽은 코드가 뒤집는다 — 두 장씩 그리면
  장수가 두 배가 되고, 그 둘이 미묘하게 달라진다
- 파일명은 아래 목록의 이름 그대로. 폴더도 그대로

### 그림체 고정

따로 만들면 매번 다른 그림체가 된다. 아래를 고정한다.

- **시점** 살짝 위에서 내려다본 정면(3/4). 움직이는 방향과 상관없이 카메라를 본다.
  **매대·가구도 마찬가지다** — 마름모 각도(윗면+두 옆면 상자)는 바닥과 건물만
  따르고, 그 위에 놓이는 것은 전부 정면 3/4로 그린다. 상자로 그리면 마당의
  어느 벽에 붙느냐에 따라 ↙판·↘판 두 장이 필요해진다(동물의 숲이 이 방식이다 —
  세계는 기울어도 가구는 카메라를 본다. 「고양이와 스프」도 같다)
- **광원** 왼쪽 위. 그림자는 오른쪽 아래
- **테두리** 굵은 갈색 선. 검정은 쓰지 않는다
- **팔레트** 아래 색에서만 고른다

```
한지  #ece2cb    흙벽  #dccfb2    나무  #8a6a45 / #6d5236
기와  #4a4139    먹    #2b241b    금    #a8763e
풀    #b3cc88    길    #efe0b4    비취  #4a7c59
```

**톤은 "볕 좋은 장날"이다** (2026-08-25에 바꿈 — 전에는 풀 #8b9e74, 길 #d9cba9로
우중충했다). 기준 그림은 까치 카드: 파란 하늘, 볕 받은 돌담. **카드 속 세상과
마을이 같은 세상으로 보여야 한다.** 갈색은 건물·소품(화면의 3할)까지만 —
바닥(6할)은 밝게, 튀는 색(단청 빨강·비취)은 눌러야 하는 것에만.

### 그림 그리는 쪽(GPT)에게 — 시점과 방향, 이 두 가지만 꼭

**1. 매대·가구는 상자(육면체)로 그리지 말 것.**

이 게임의 바닥은 마름모(비스듬히 내려다본 격자)다. 마름모 위에 상자를
그리면 윗면과 두 옆면이 보이는데, 그러면 상자의 "정면"이 왼쪽 아래(↙)를
보거나 오른쪽 아래(↘)를 보거나 **둘 중 하나**여야 한다. 매대는 마당의
왼쪽 벽에도 오른쪽 벽에도 놓이므로, 상자로 그리면 **↙판·↘판 두 장씩**
필요해진다 — 실제로 지난번 매대가 상자로 와서, 반대쪽 벽에 놓인 것이
어색했다.

그래서 규칙은 이거다: **마름모 각도는 바닥과 건물만 따른다. 그 위에 놓이는
것(매대·물건·인물·손님)은 전부 '살짝 위에서 내려다본 정면(3/4)'으로
그린다.** 동물의 숲이 이 방식이다 — 세계는 기울어도 가구는 카메라를 본다.
매대라면: 낮은 좌판을 거의 정면에서, 윗면이 살짝 보이게. 그러면 한 장으로
어느 벽에나 놓인다.

**3. 그림자를 그림에 넣지 말 것.** 발밑 그림자는 코드가 납작한 원으로
그린다. 그림에 구워 넣으면 동물×방향마다 그림자가 딸려가고, 걸으며 몸이
들썩일 때 그림자까지 같이 떠 버린다 — 그림자는 땅에 붙어 있어야 발이 땅을
딛는 느낌이 난다. **배경 투명, 발끝까지만** 그리면 된다.

**4. 발끝은 아래 변에, 몸은 가로 정중앙에.** 서 있는 짐승 그림은 발끝이
캔버스 아래 변에 닿고, 몸의 가로 중심이 캔버스 정중앙이어야 한다.
어긋나면 발이 공중에 뜨고 그림자가 밀려 보인다. 코드가 재서 받아주긴
하지만 그건 안전망이다 — 검사는 `godot --headless --path godot --script
tests/artfit.gd`가 어긋난 장을 자동으로 집어준다.

**5. 마당의 담(울타리 벽)은 그리지 말 것.** 담은 코드가 그린다 — 초가(짚) →
돌담 → 기와로 가게 등급 따라 저절로 갈아입는다. 담은 길이가 마당 크기마다
달라서 그림으로 받으면 늘여 붙이는 코드부터 새로 짜야 한다. 언젠가 그림 담이
필요해지면 그때 규격(이어 붙일 한 토막 크기)을 정해서 여기 목록에 올린다.

**2. 걷는 동물은 세 방향이다 — 옆·정면·뒷모습. 좌우는 코드가 뒤집는다.**

(처음엔 옆모습 한 장으로 때우자고 했다가 만드는 사람이 되돌렸다.)

- `<id>.png` — **옆모습**(오른쪽 보기). 걷는 대부분을 맡는다. 왼쪽은 코드가 뒤집는다
- `<id>-front.png` — **정면.** 줄에 서서 계산할 때 카메라를 본다
- `<id>-back.png` — **뒷모습.** 화면 위로 걸어 올라갈 때

없는 방향은 옆모습으로 때우니 **흔한 손님부터 한 장씩** 넣으면 된다.
네 방향을 다 그릴 필요는 없다 — 좌우는 언제나 코드 뒤집기다.

### 만드는 방법

**한 장에 몰아서 그린 뒤 잘라 쓴다.** 한 번에 그리면 같은 붓으로 그려져
저절로 통일된다. 따로 12번 시키면 12개 그림체가 나온다.

## 어느 순서로 그리나

| 차례 | 무엇 | 장수 | 왜 이 순서인가 |
|---|---|---|---|
| **1차** | 손님 30종 (걷는 모습) | **30** | 마을이 회색 덩어리를 벗는다. 지금 제일 어색한 자리 |
| **2차** | 손님 카드 1단 30장 | **30** | 뽑기에서 크게 뜨고 도감에 모인다 — **제일 오래 들여다보는 그림** |
| **3차** | 점장 완성형 — 가게 15 × (옆+뒷) | **30** | 가게 정체성이 살아난다 — 대장간은 망치 든 너구리 |
| **4차** | 직원 너구리 4등급 (두건) | **8** | 공용 몸에 **머리에 쓴 것만** 다르다 — 머리띠 → 흰 두건 → 회색 두건 → 검정 두건 |
| **5차** | 물건 40종 | **40** | 제일 많다. 매대와 가게 창 두 군데에 쓰인다 |
| **6차** | 매대 5종 · 나쁜 놈 3종 | **8** | 가게마다 좌판이 달라진다 |
| 선택 | 손님 카드 2~4단 | 90 | 성이 오를 때 갈아입는다. 없으면 1단을 계속 쓴다 |
| 선택 | 점장 승급판·조는 모습 | 105 | `-side-1/-2` 등 — 없으면 1등급 그림을 계속 쓴다 |

### 누가 누구인지 — 이게 제일 중요하다

**점장은 가게마다 완성형으로 그린다**(2026-08-26 확정 — 겹그림 조립 계약은
접었다). 통통 너구리 몸(HERO-A 그림체 기준)에 그 가게의 옷·연장·꼬리까지
**한 장에 통합**해서:

```
clerks/<가게>-side.png   오른쪽 훼이크 측면 — 일·계산·서기 전부 이 한 장
clerks/<가게>-back.png   뒷모습 — 위로 걸을 때
(선택) -sleep · 승급판 -side-1/-2 등
```

- **정면은 없다.** 서 있을 땐 마지막 방향 그대로 멈추고, 할 일 없으면 존다.
- 왼쪽은 코드가 뒤집는다. 움직임(걸음 통통·숨 말캉임)도 코드가 만든다.
- 승급하면 그림이 통째로 갈린다(-1·-2) — 없으면 1등급 그림을 계속 쓴다.
- 카드 초상(cards/)은 별도 그림이다 — 마당 그림과 무관하게 그대로.

| | 누구 | 어떻게 그리나 |
|---|---|---|
| **점장** | 이 가게 주인. 만들고 팔고 계산대를 오간다 | 가게별 완성형 — 몸+옷+연장 한 장 |
| **직원** | 고용된 쪽. 매대 곁에서 만들기만 한다 | 공용 몸 + 두건만 4등급 |
| **손님** | 사러 온 짐승들. **서른 종** | 너구리가 아니다. 토끼·까치·해태… |

★ 방향 문법: **손님은 진짜 세 방향**(옆·정면·뒷)이고, **점장은 둘**(옆·뒷)
이다. 대각선 이동은 옆(↘↙=뒤집기)과 뒷(↖↗)으로 다 덮인다 — 정면 몸을
따로 그릴 필요가 없는 이유다.

### 손님은 그림이 **두 벌**이다 — 헷갈리기 쉬운 자리

| | 어디에 쓰나 | 크기 | 어떻게 다른가 |
|---|---|---|---|
| `guests/rabbit.png` | 마을 길을 걸어온다 | 128×128 | **작다.** 옆에서 본 걷는 모습, 배경 없음 |
| `cards/rabbit-1.png` | 뽑기·도감 카드 | 512×768 | **크다.** 정면으로 선 초상, 배경까지 |

카드는 이 게임에서 **제일 오래 들여다보는 그림**이다. 뽑을 때 화면 가운데에
크게 뜨고, 도감에 모아 두고 다시 본다. 걷는 모습은 48px로 작게 지나가지만
카드는 250px로 멈춰 서 있으니 **공은 카드 쪽에 더 들여도 된다.**

카드 테두리와 등급 표시는 **코드가 그린다.** 짐승과 배경만 그리면 된다.

### 등급 여섯 — 귀할수록 귀티가 나야 한다

| | 등급 | 몇 종 | 어떤 짐승 |
|---|---|---|---|
| ⚪ | 흔함 | 8 | 토끼 · 까치 · 다람쥐 · 참새 · 개구리 · 두더지 · 고슴도치 · 오리 |
| 🟢 | 드묾 | 7 | 오소리 · 여우 · 수달 · 노루 · 족제비 · 살쾡이 · 산양 |
| 🔵 | 귀함 | 6 | 사슴 · 멧돼지 · 담비 · 원앙 · 늑대 · 백로 |
| 🟣 | 진귀 | 5 | 곰 · 거북 · 표범 · 사향노루 · 반달곰 |
| 🟠 | 영물 | 3 | 두루미 · 소 · **해태** |
| 🔴 | 신수 | 1 | **호랑이** |

**등급이 그림에서 읽혀야 한다.** 흔함은 수수하게, 신수는 한눈에 다르게.
같은 붓으로 그리되 공들인 정도가 달라야 뽑았을 때 "좋은 게 나왔다"가 온다.

**어디서 멈춰도 게임은 돈다.** 그림이 없는 자리는 여태처럼 도형과 이모지로
그린다. 한 장 넣을 때마다 그 자리만 바뀐다 — 전부 모을 때까지 기다릴 필요가 없다.
지금 세 장(점장 둘·토끼)이 이미 그렇게 화면에 나오고 있다.

## 복사해서 GPT에 붙여넣는 주문 문안 — 프로필 초상

배치를 말로 어떻게 설명하나 막막하면 **이 문안을 통째로 붙여넣으면 된다.**
게임을 만들어 봐야 쓸 수 있는 말은 하나도 없다 — 숫자 세 개가 배치의 전부다.

```
모바일 게임의 동그란 프로필 초상을 그려줘. 사람 SNS 프로필 사진의
동물 판이라고 생각하면 된다.

[규격]
- 256×256 픽셀, PNG, 배경 완전 투명
- 그림은 동그라미 안에만 그린다. 원 밖은 아무것도 없이 투명
- 동물의 어깨까지만(상반신). 몸통·다리는 그리지 않는다
- 얼굴이 원 크기의 7할을 차지할 만큼 크게. 증명사진처럼
- 머리 꼭대기와 원 가장자리 사이는 조금만 띄운다(원 지름의 1할)
- 정면을 보되 카메라가 살짝 위에 있는 각도

[그림체 — 반드시 지킬 것]
- 부드러운 만화풍, 굵은 갈색 테두리 선(검정 금지)
- 따뜻한 낮 볕 느낌. 색은 첨부한 그림의 팔레트를 따른다
- 그림자, 글자, 장식 테두리, 배경 무늬는 넣지 않는다
  (동그란 테두리 띠는 게임이 알아서 두른다)

[첨부 그림이 있으면]
같은 동물의 카드 그림을 첨부한다. 새로 창작하지 말고 이 그림의
얼굴을 위 규격대로 다시 앉혀라 — 같은 표정, 같은 색, 같은 그림체.

이번 장: ○○○ (파일 이름: portraits/○○○.png)
```

쓰는 법: 마지막 줄의 ○○○만 바꾼다(동물 이름과 파일 이름은 아래 목록에
있다). 그 동물의 카드 그림(`godot/art/cards/`)이 있으면 **꼭 같이 첨부**한다 —
그래야 서른 장의 그림체가 흩어지지 않는다. 첫 장은 주인공 너구리
(`portraits/raccoon.png`, 카드 대신 `hero/raccoon-make.png` 첨부)를 추천 —
제일 자주 보이는 얼굴이라 이 한 장으로 그림체 기준을 잡고 가는 게 좋다.

**남은 장수와 파일 이름은 아래 목록에 있다** — 폴더를 훑어서 자동으로 세므로
절대 안 어긋난다. 갱신: `node tools/art.mjs --write`

---

## 남은 그림

<!-- 목록시작 -->
> 이 칸은 `node tools/art.mjs --write`가 다시 쓴다.
> **그림을 폴더에 넣으면 목록에서 저절로 빠진다** — 손으로 지울 필요 없다.

**164 / 577장** 들어왔다.

### 점장 너구리 (폴백) · 촌장 — `godot/art/hero/` · 144×144 · **0장 남음**

전부 들어왔다. ✅

### 물건 — `godot/art/items/` · 128×224 · **40장 남음**

**두 군데에 쓰인다** — 매대 위(원래 크기)와 가게 창의 목록 썸네일(작게 줄여서).
그래서 작게 줄여도 뭔지 알아볼 수 있어야 한다 — 잔무늬보다 **실루엣**이 중요하다.

**등급 그림은 그리고 싶은 것만.** 가게를 승급하면 물건 이름이 바뀐다
(무쇠곡괭이 → 참쇠곡괭이 → 강철곡괭이). 그 등급 그림을 따로 주고 싶으면
`pick-1.png`(2등급) · `pick-2.png`(3등급)로 넣으면 그것부터 쓴다.
★ 승급판은 이제 **필수 목록**(아래 "물건 승급판")에 있다 — 등급 테를
둘러 표시하던 것은 지웠다(판타지 RPG 같다는 유저 결정). 그림이 곧 등급이다.
40종 × 3등급 = 120장을 다 그릴 이유는 없다.

- [ ] `gulbi.png` — 어물전 · 굴비 (지금 🐟)
- [ ] `miyeok.png` — 어물전 · 미역 (지금 🌿)
- [ ] `jeotgal.png` — 어물전 · 젓갈 (지금 🫙)
- [ ] `bugeo.png` — 어물전 · 북어 (지금 🎣)
- [ ] `gim.png` — 어물전 · 김 (지금 🍙)
- [ ] `jaban.png` — 어물전 · 자반 (지금 🐠)
- [ ] `muneo.png` — 어물전 · 문어 (지금 🐙)
- [ ] `jeonbok.png` — 어물전 · 전복 (지금 🦪)
- [ ] `mumyeong.png` — 포목전 · 무명 (지금 🧵)
- [ ] `sambe.png` — 포목전 · 삼베 (지금 🪢)
- [ ] `mosi.png` — 포목전 · 모시 (지금 🎽)
- [ ] `myeongju.png` — 포목전 · 명주 (지금 🧣)
- [ ] `yeomnang.png` — 포목전 · 염낭 (지금 👛)
- [ ] `bidan.png` — 포목전 · 비단 (지금 🎀)
- [ ] `gwanbok.png` — 포목전 · 관복감 (지금 🥻)
- [ ] `hollye.png` — 포목전 · 혼례비단 (지금 💒)
- [ ] `satgat.png` — 갓방 · 삿갓 (지금 👒)
- [ ] `manggeon.png` — 갓방 · 망건 (지금 🪖)
- [ ] `tanggeon.png` — 갓방 · 탕건 (지금 🎩)
- [ ] `gatkkeun.png` — 갓방 · 갓끈 (지금 📿)
- [ ] `heungnip.png` — 갓방 · 흑립 (지금 🎓)
- [ ] `jurip.png` — 갓방 · 주립 (지금 ⛑️)
- [ ] `jeongjagwan.png` — 갓방 · 정자관 (지금 👑)
- [ ] `oknorip.png` — 갓방 · 옥로립 (지금 💎)
- [ ] `notsujeo.png` — 유기전 · 놋수저 (지금 🥄)
- [ ] `notjubal.png` — 유기전 · 놋주발 (지금 🥣)
- [ ] `notjaengban.png` — 유기전 · 놋쟁반 (지금 🍽️)
- [ ] `notchotdae.png` — 유기전 · 놋촛대 (지금 🕯️)
- [ ] `nothwaro.png` — 유기전 · 놋화로 (지금 🔥)
- [ ] `notdaeya.png` — 유기전 · 놋대야 (지금 🛁)
- [ ] `jegi.png` — 유기전 · 제기 (지금 ⚱️)
- [ ] `bansangki.png` — 유기전 · 반상기 (지금 🍱)
- [ ] `najeonbit.png` — 나전방 · 나전빗 (지금 💈)
- [ ] `gyeongdae.png` — 나전방 · 경대 (지금 🪞)
- [ ] `ham.png` — 나전방 · 함 (지금 🎁)
- [ ] `mungap.png` — 나전방 · 문갑 (지금 🗄️)
- [ ] `jwagyeong.png` — 나전방 · 좌경 (지금 🔍)
- [ ] `samcheungjang.png` — 나전방 · 삼층장 (지금 🪜)
- [ ] `byeongpung.png` — 나전방 · 자개병풍 (지금 🖼️)
- [ ] `najeonnong.png` — 나전방 · 나전롱 (지금 👘)

### 물건 승급판 (2·3등급) — `godot/art/items/` · 128×224 · **240장 남음**

**등급 테두리(은·금 고리)를 지웠다** — 승급은 물건 그림 자체가 바뀌는 것으로 보여준다.
같은 물건이 격이 오른 모습으로: 무쇠도끼 → 참쇠도끼(날이 서고) → 강철도끼(광이 난다).
파일명은 `<물건id>-1.png`(2등급) · `<물건id>-2.png`(3등급).

- [ ] `pick-1.png` — 참쇠곡괭이 (대장간)
- [ ] `pick-2.png` — 강철곡괭이 (대장간)
- [ ] `sickle-1.png` — 참쇠낫 (대장간)
- [ ] `sickle-2.png` — 강철낫 (대장간)
- [ ] `hoe-1.png` — 참쇠호미 (대장간)
- [ ] `hoe-2.png` — 강철호미 (대장간)
- [ ] `axe-1.png` — 참쇠도끼 (대장간)
- [ ] `axe-2.png` — 강철도끼 (대장간)
- [ ] `shears-1.png` — 참쇠가위 (대장간)
- [ ] `shears-2.png` — 강철가위 (대장간)
- [ ] `knife-1.png` — 참쇠부엌칼 (대장간)
- [ ] `knife-2.png` — 강철부엌칼 (대장간)
- [ ] `lock-1.png` — 참쇠자물쇠 (대장간)
- [ ] `lock-2.png` — 강철자물쇠 (대장간)
- [ ] `cauldr-1.png` — 참쇠가마솥 (대장간)
- [ ] `cauldr-2.png` — 강철가마솥 (대장간)
- [ ] `brush-1.png` — 고운붓 (필방)
- [ ] `brush-2.png` — 명품붓 (필방)
- [ ] `ink-1.png` — 고운먹 (필방)
- [ ] `ink-2.png` — 명품먹 (필방)
- [ ] `inkstone-1.png` — 고운벼루 (필방)
- [ ] `inkstone-2.png` — 명품벼루 (필방)
- [ ] `waterp-1.png` — 고운연적 (필방)
- [ ] `waterp-2.png` — 명품연적 (필방)
- [ ] `brushpot-1.png` — 고운필통 (필방)
- [ ] `brushpot-2.png` — 명품필통 (필방)
- [ ] `bookmk-1.png` — 고운서산 (필방)
- [ ] `bookmk-2.png` — 명품서산 (필방)
- [ ] `brushrk-1.png` — 고운붓걸이 (필방)
- [ ] `brushrk-2.png` — 명품붓걸이 (필방)
- [ ] `album-1.png` — 고운화첩 (필방)
- [ ] `album-2.png` — 명품화첩 (필방)
- [ ] `hanji-1.png` — 고운한지 (지물포)
- [ ] `hanji-2.png` — 진상한지 (지물포)
- [ ] `fan-1.png` — 고운부채 (지물포)
- [ ] `fan-2.png` — 진상부채 (지물포)
- [ ] `window-1.png` — 고운창호지 (지물포)
- [ ] `window-2.png` — 진상창호지 (지물포)
- [ ] `floorp-1.png` — 고운장지 (지물포)
- [ ] `floorp-2.png` — 진상장지 (지물포)
- [ ] `kite-1.png` — 고운연 (지물포)
- [ ] `kite-2.png` — 진상연 (지물포)
- [ ] `umbrel-1.png` — 고운지우산 (지물포)
- [ ] `umbrel-2.png` — 진상지우산 (지물포)
- [ ] `lantrn-1.png` — 고운지등 (지물포)
- [ ] `lantrn-2.png` — 진상지등 (지물포)
- [ ] `screen-1.png` — 고운병풍 (지물포)
- [ ] `screen-2.png` — 진상병풍 (지물포)
- [ ] `jar-1.png` — 오지옹기 (옹기점)
- [ ] `jar-2.png` — 왕실옹기 (옹기점)
- [ ] `bowl-1.png` — 오지사발 (옹기점)
- [ ] `bowl-2.png` — 왕실사발 (옹기점)
- [ ] `celad-1.png` — 오지청자 (옹기점)
- [ ] `celad-2.png` — 왕실청자 (옹기점)
- [ ] `steamr-1.png` — 오지시루 (옹기점)
- [ ] `steamr-2.png` — 왕실시루 (옹기점)
- [ ] `bottle-1.png` — 오지술병 (옹기점)
- [ ] `bottle-2.png` — 왕실술병 (옹기점)
- [ ] `teaset-1.png` — 오지다기 (옹기점)
- [ ] `teaset-2.png` — 왕실다기 (옹기점)
- [ ] `censer-1.png` — 오지향로 (옹기점)
- [ ] `censer-2.png` — 왕실향로 (옹기점)
- [ ] `moonjr-1.png` — 오지달항아리 (옹기점)
- [ ] `moonjr-2.png` — 왕실달항아리 (옹기점)
- [ ] `root-1.png` — 묵은도라지 (약재상)
- [ ] `root-2.png` — 천년도라지 (약재상)
- [ ] `ginseng-1.png` — 묵은산삼 (약재상)
- [ ] `ginseng-2.png` — 천년산삼 (약재상)
- [ ] `antler-1.png` — 묵은녹용 (약재상)
- [ ] `antler-2.png` — 천년녹용 (약재상)
- [ ] `bezoar-1.png` — 묵은우황 (약재상)
- [ ] `bezoar-2.png` — 천년우황 (약재상)
- [ ] `danggui-1.png` — 묵은당귀 (약재상)
- [ ] `danggui-2.png` — 천년당귀 (약재상)
- [ ] `reishi-1.png` — 묵은영지 (약재상)
- [ ] `reishi-2.png` — 천년영지 (약재상)
- [ ] `agar-1.png` — 묵은침향 (약재상)
- [ ] `agar-2.png` — 천년침향 (약재상)
- [ ] `elixir-1.png` — 묵은경옥고 (약재상)
- [ ] `elixir-2.png` — 천년경옥고 (약재상)
- [ ] `bap-1.png` — 곰국국밥 (국밥집)
- [ ] `bap-2.png` — 용봉탕국밥 (국밥집)
- [ ] `kuk-1.png` — 곰국장국 (국밥집)
- [ ] `kuk-2.png` — 용봉탕장국 (국밥집)
- [ ] `sujebi-1.png` — 곰국수제비 (국밥집)
- [ ] `sujebi-2.png` — 용봉탕수제비 (국밥집)
- [ ] `naeng-1.png` — 곰국냉국 (국밥집)
- [ ] `naeng-2.png` — 용봉탕냉국 (국밥집)
- [ ] `gomtang-1.png` — 곰국곰탕 (국밥집)
- [ ] `gomtang-2.png` — 용봉탕곰탕 (국밥집)
- [ ] `samgye-1.png` — 곰국삼계탕 (국밥집)
- [ ] `samgye-2.png` — 용봉탕삼계탕 (국밥집)
- [ ] `chueo-1.png` — 곰국추어탕 (국밥집)
- [ ] `chueo-2.png` — 용봉탕추어탕 (국밥집)
- [ ] `yongbong-1.png` — 곰국용봉탕 (국밥집)
- [ ] `yongbong-2.png` — 용봉탕용봉탕 (국밥집)
- [ ] `makgeol-1.png` — 청막걸리 (객주)
- [ ] `makgeol-2.png` — 법막걸리 (객주)
- [ ] `jeon-1.png` — 청파전 (객주)
- [ ] `jeon-2.png` — 법파전 (객주)
- [ ] `dongdong-1.png` — 청동동주 (객주)
- [ ] `dongdong-2.png` — 법동동주 (객주)
- [ ] `muk-1.png` — 청묵무침 (객주)
- [ ] `muk-2.png` — 법묵무침 (객주)
- [ ] `cheongju-1.png` — 청청주 (객주)
- [ ] `cheongju-2.png` — 법청주 (객주)
- [ ] `bossam-1.png` — 청보쌈 (객주)
- [ ] `bossam-2.png` — 법보쌈 (객주)
- [ ] `beopju-1.png` — 청법주 (객주)
- [ ] `beopju-2.png` — 법법주 (객주)
- [ ] `gujeol-1.png` — 청구절판 (객주)
- [ ] `gujeol-2.png` — 법구절판 (객주)
- [ ] `tteokggo-1.png` — 참숯떡꼬치 (꼬치집)
- [ ] `tteokggo-2.png` — 백탄떡꼬치 (꼬치집)
- [ ] `dakggo-1.png` — 참숯닭꼬치 (꼬치집)
- [ ] `dakggo-2.png` — 백탄닭꼬치 (꼬치집)
- [ ] `beoseot-1.png` — 참숯버섯꼬치 (꼬치집)
- [ ] `beoseot-2.png` — 백탄버섯꼬치 (꼬치집)
- [ ] `saengseon-1.png` — 참숯생선꼬치 (꼬치집)
- [ ] `saengseon-2.png` — 백탄생선꼬치 (꼬치집)
- [ ] `sanjeok-1.png` — 참숯산적 (꼬치집)
- [ ] `sanjeok-2.png` — 백탄산적 (꼬치집)
- [ ] `jangeo-1.png` — 참숯장어구이 (꼬치집)
- [ ] `jangeo-2.png` — 백탄장어구이 (꼬치집)
- [ ] `neobiani-1.png` — 참숯너비아니 (꼬치집)
- [ ] `neobiani-2.png` — 백탄너비아니 (꼬치집)
- [ ] `yukhoe-1.png` — 참숯육회 (꼬치집)
- [ ] `yukhoe-2.png` — 백탄육회 (꼬치집)
- [ ] `garae-1.png` — 찹쌀가래떡 (떡집)
- [ ] `garae-2.png` — 진상가래떡 (떡집)
- [ ] `injeol-1.png` — 찹쌀인절미 (떡집)
- [ ] `injeol-2.png` — 진상인절미 (떡집)
- [ ] `songpyeon-1.png` — 찹쌀송편 (떡집)
- [ ] `songpyeon-2.png` — 진상송편 (떡집)
- [ ] `baekseol-1.png` — 찹쌀백설기 (떡집)
- [ ] `baekseol-2.png` — 진상백설기 (떡집)
- [ ] `yaksik-1.png` — 찹쌀약식 (떡집)
- [ ] `yaksik-2.png` — 진상약식 (떡집)
- [ ] `hwajeon-1.png` — 찹쌀화전 (떡집)
- [ ] `hwajeon-2.png` — 진상화전 (떡집)
- [ ] `dasik-1.png` — 찹쌀다식 (떡집)
- [ ] `dasik-2.png` — 진상다식 (떡집)
- [ ] `yugwa-1.png` — 찹쌀유과 (떡집)
- [ ] `yugwa-2.png` — 진상유과 (떡집)
- [ ] `dwaeji-1.png` — 상돼지고기 (푸줏간)
- [ ] `dwaeji-2.png` — 진상돼지고기 (푸줏간)
- [ ] `dak-1.png` — 상닭고기 (푸줏간)
- [ ] `dak-2.png` — 진상닭고기 (푸줏간)
- [ ] `sogogi-1.png` — 상쇠고기 (푸줏간)
- [ ] `sogogi-2.png` — 진상쇠고기 (푸줏간)
- [ ] `galbi-1.png` — 상갈비 (푸줏간)
- [ ] `galbi-2.png` — 진상갈비 (푸줏간)
- [ ] `ugeoji-1.png` — 상우거지 (푸줏간)
- [ ] `ugeoji-2.png` — 진상우거지 (푸줏간)
- [ ] `gopchang-1.png` — 상곱창 (푸줏간)
- [ ] `gopchang-2.png` — 진상곱창 (푸줏간)
- [ ] `ansim-1.png` — 상안심 (푸줏간)
- [ ] `ansim-2.png` — 진상안심 (푸줏간)
- [ ] `hanwoo-1.png` — 상한우 등심 (푸줏간)
- [ ] `hanwoo-2.png` — 진상한우 등심 (푸줏간)
- [ ] `gulbi-1.png` — 말린굴비 (어물전)
- [ ] `gulbi-2.png` — 진상굴비 (어물전)
- [ ] `miyeok-1.png` — 말린미역 (어물전)
- [ ] `miyeok-2.png` — 진상미역 (어물전)
- [ ] `jeotgal-1.png` — 말린젓갈 (어물전)
- [ ] `jeotgal-2.png` — 진상젓갈 (어물전)
- [ ] `bugeo-1.png` — 말린북어 (어물전)
- [ ] `bugeo-2.png` — 진상북어 (어물전)
- [ ] `gim-1.png` — 말린김 (어물전)
- [ ] `gim-2.png` — 진상김 (어물전)
- [ ] `jaban-1.png` — 말린자반 (어물전)
- [ ] `jaban-2.png` — 진상자반 (어물전)
- [ ] `muneo-1.png` — 말린문어 (어물전)
- [ ] `muneo-2.png` — 진상문어 (어물전)
- [ ] `jeonbok-1.png` — 말린전복 (어물전)
- [ ] `jeonbok-2.png` — 진상전복 (어물전)
- [ ] `mumyeong-1.png` — 명주무명 (포목전)
- [ ] `mumyeong-2.png` — 비단무명 (포목전)
- [ ] `sambe-1.png` — 명주삼베 (포목전)
- [ ] `sambe-2.png` — 비단삼베 (포목전)
- [ ] `mosi-1.png` — 명주모시 (포목전)
- [ ] `mosi-2.png` — 비단모시 (포목전)
- [ ] `myeongju-1.png` — 명주명주 (포목전)
- [ ] `myeongju-2.png` — 비단명주 (포목전)
- [ ] `yeomnang-1.png` — 명주염낭 (포목전)
- [ ] `yeomnang-2.png` — 비단염낭 (포목전)
- [ ] `bidan-1.png` — 명주비단 (포목전)
- [ ] `bidan-2.png` — 비단비단 (포목전)
- [ ] `gwanbok-1.png` — 명주관복감 (포목전)
- [ ] `gwanbok-2.png` — 비단관복감 (포목전)
- [ ] `hollye-1.png` — 명주혼례비단 (포목전)
- [ ] `hollye-2.png` — 비단혼례비단 (포목전)
- [ ] `satgat-1.png` — 중삿갓 (갓방)
- [ ] `satgat-2.png` — 진상삿갓 (갓방)
- [ ] `manggeon-1.png` — 중망건 (갓방)
- [ ] `manggeon-2.png` — 진상망건 (갓방)
- [ ] `tanggeon-1.png` — 중탕건 (갓방)
- [ ] `tanggeon-2.png` — 진상탕건 (갓방)
- [ ] `gatkkeun-1.png` — 중갓끈 (갓방)
- [ ] `gatkkeun-2.png` — 진상갓끈 (갓방)
- [ ] `heungnip-1.png` — 중흑립 (갓방)
- [ ] `heungnip-2.png` — 진상흑립 (갓방)
- [ ] `jurip-1.png` — 중주립 (갓방)
- [ ] `jurip-2.png` — 진상주립 (갓방)
- [ ] `jeongjagwan-1.png` — 중정자관 (갓방)
- [ ] `jeongjagwan-2.png` — 진상정자관 (갓방)
- [ ] `oknorip-1.png` — 중옥로립 (갓방)
- [ ] `oknorip-2.png` — 진상옥로립 (갓방)
- [ ] `notsujeo-1.png` — 상놋수저 (유기전)
- [ ] `notsujeo-2.png` — 왕실놋수저 (유기전)
- [ ] `notjubal-1.png` — 상놋주발 (유기전)
- [ ] `notjubal-2.png` — 왕실놋주발 (유기전)
- [ ] `notjaengban-1.png` — 상놋쟁반 (유기전)
- [ ] `notjaengban-2.png` — 왕실놋쟁반 (유기전)
- [ ] `notchotdae-1.png` — 상놋촛대 (유기전)
- [ ] `notchotdae-2.png` — 왕실놋촛대 (유기전)
- [ ] `nothwaro-1.png` — 상놋화로 (유기전)
- [ ] `nothwaro-2.png` — 왕실놋화로 (유기전)
- [ ] `notdaeya-1.png` — 상놋대야 (유기전)
- [ ] `notdaeya-2.png` — 왕실놋대야 (유기전)
- [ ] `jegi-1.png` — 상제기 (유기전)
- [ ] `jegi-2.png` — 왕실제기 (유기전)
- [ ] `bansangki-1.png` — 상반상기 (유기전)
- [ ] `bansangki-2.png` — 왕실반상기 (유기전)
- [ ] `najeonbit-1.png` — 진주나전빗 (나전방)
- [ ] `najeonbit-2.png` — 왕실나전빗 (나전방)
- [ ] `gyeongdae-1.png` — 진주경대 (나전방)
- [ ] `gyeongdae-2.png` — 왕실경대 (나전방)
- [ ] `ham-1.png` — 진주함 (나전방)
- [ ] `ham-2.png` — 왕실함 (나전방)
- [ ] `mungap-1.png` — 진주문갑 (나전방)
- [ ] `mungap-2.png` — 왕실문갑 (나전방)
- [ ] `jwagyeong-1.png` — 진주좌경 (나전방)
- [ ] `jwagyeong-2.png` — 왕실좌경 (나전방)
- [ ] `samcheungjang-1.png` — 진주삼층장 (나전방)
- [ ] `samcheungjang-2.png` — 왕실삼층장 (나전방)
- [ ] `byeongpung-1.png` — 진주자개병풍 (나전방)
- [ ] `byeongpung-2.png` — 왕실자개병풍 (나전방)
- [ ] `najeonnong-1.png` — 진주나전롱 (나전방)
- [ ] `najeonnong-2.png` — 왕실나전롱 (나전방)

### 직원 너구리 — `godot/art/staff/` · 144×144 · **6장 남음**

**공용 몸 한 벌이다 — 점장 무늬(a~d) 배정과 무관하다.** 크기는 점장과
같다(크기로 가르면 덜 자란 너구리처럼 보인다). 가르는 것은 **머리에 쓴 것**뿐.
등급도 모자로만 가른다 — 몸은 네 등급이 똑같아도 된다. 위로 갈수록 격이 오른다.
`work`는 일하는 자세, `sleep`은 조는 자세 — 진열대가 다 차면 존다.
★ **틀 안 채움을 점장과 똑같이** — 지금 직원 그림이 점장보다 작게 차 있어
화면에서 몸집이 달라 보인다(코드 크기는 둘 다 72로 같다). 발끝~정수리가
틀의 9할을 차게 그리면 같은 몸집이 된다. 등급 이름이 바뀌었다:
머리띠 → 흰 두건 → 회색 두건 → 검정 두건(whitehood·grayhood·blackhood).

- [ ] `whitehood-work.png` — 흰 두건 · 흰 무명 두건 — 일하는 중
- [ ] `whitehood-sleep.png` — 흰 두건 · 흰 무명 두건 — 조는 중 — 만들 데가 없다
- [ ] `grayhood-work.png` — 회색 두건 · 잿빛 두건 — 일하는 중
- [ ] `grayhood-sleep.png` — 회색 두건 · 잿빛 두건 — 조는 중 — 만들 데가 없다
- [ ] `blackhood-work.png` — 검정 두건 · 검정 두건 — 제일 높다 — 일하는 중
- [ ] `blackhood-sleep.png` — 검정 두건 · 검정 두건 — 제일 높다 — 조는 중 — 만들 데가 없다

### 손님 — `godot/art/guests/` · 128×128 · **0장 남음**

마을에 걸어 들어오는 모습. **카드 그림과 다르다** — 이건 옆에서 본 걷는 모습이고,
카드는 정면으로 선 초상이다. 여기부터 있어야 마을이 회색 덩어리를 벗는다.
괄호 안이 카드 등급이다 — 귀한 짐승일수록 귀티가 나야 한다.

전부 들어왔다. ✅

### 손님 정면·뒷모습 — `godot/art/guests/` · 128×128 · **60장 남음**

**옆모습 한 장으로 때우려던 것을 되돌렸다**(만드는 사람 결정).
`-front`(정면) — 줄에 서서 계산할 때 카메라를 본다.
`-back`(뒷모습) — 화면 위로 걸어 올라갈 때. 좌우는 여전히 코드가 뒤집는다.
없는 방향은 옆모습으로 때우니, 흔한 손님부터 한 장씩 넣으면 된다.

- [ ] `rabbit-front.png` — 토끼 정면 — 줄 설 때
- [ ] `rabbit-back.png` — 토끼 뒷모습 — 올라갈 때
- [ ] `magpie-front.png` — 까치 정면 — 줄 설 때
- [ ] `magpie-back.png` — 까치 뒷모습 — 올라갈 때
- [ ] `squirrel-front.png` — 다람쥐 정면 — 줄 설 때
- [ ] `squirrel-back.png` — 다람쥐 뒷모습 — 올라갈 때
- [ ] `badger-front.png` — 오소리 정면 — 줄 설 때
- [ ] `badger-back.png` — 오소리 뒷모습 — 올라갈 때
- [ ] `fox-front.png` — 여우 정면 — 줄 설 때
- [ ] `fox-back.png` — 여우 뒷모습 — 올라갈 때
- [ ] `deer-front.png` — 사슴 정면 — 줄 설 때
- [ ] `deer-back.png` — 사슴 뒷모습 — 올라갈 때
- [ ] `boar-front.png` — 멧돼지 정면 — 줄 설 때
- [ ] `boar-back.png` — 멧돼지 뒷모습 — 올라갈 때
- [ ] `bear-front.png` — 곰 정면 — 줄 설 때
- [ ] `bear-back.png` — 곰 뒷모습 — 올라갈 때
- [ ] `turtle-front.png` — 거북 정면 — 줄 설 때
- [ ] `turtle-back.png` — 거북 뒷모습 — 올라갈 때
- [ ] `crane-front.png` — 두루미 정면 — 줄 설 때
- [ ] `crane-back.png` — 두루미 뒷모습 — 올라갈 때
- [ ] `ox-front.png` — 소 정면 — 줄 설 때
- [ ] `ox-back.png` — 소 뒷모습 — 올라갈 때
- [ ] `tiger-front.png` — 호랑이 정면 — 줄 설 때
- [ ] `tiger-back.png` — 호랑이 뒷모습 — 올라갈 때
- [ ] `sparrow-front.png` — 참새 정면 — 줄 설 때
- [ ] `sparrow-back.png` — 참새 뒷모습 — 올라갈 때
- [ ] `frog-front.png` — 개구리 정면 — 줄 설 때
- [ ] `frog-back.png` — 개구리 뒷모습 — 올라갈 때
- [ ] `mole-front.png` — 두더지 정면 — 줄 설 때
- [ ] `mole-back.png` — 두더지 뒷모습 — 올라갈 때
- [ ] `hedgehog-front.png` — 고슴도치 정면 — 줄 설 때
- [ ] `hedgehog-back.png` — 고슴도치 뒷모습 — 올라갈 때
- [ ] `duck-front.png` — 오리 정면 — 줄 설 때
- [ ] `duck-back.png` — 오리 뒷모습 — 올라갈 때
- [ ] `otter-front.png` — 수달 정면 — 줄 설 때
- [ ] `otter-back.png` — 수달 뒷모습 — 올라갈 때
- [ ] `roe-front.png` — 노루 정면 — 줄 설 때
- [ ] `roe-back.png` — 노루 뒷모습 — 올라갈 때
- [ ] `weasel-front.png` — 족제비 정면 — 줄 설 때
- [ ] `weasel-back.png` — 족제비 뒷모습 — 올라갈 때
- [ ] `wildcat-front.png` — 살쾡이 정면 — 줄 설 때
- [ ] `wildcat-back.png` — 살쾡이 뒷모습 — 올라갈 때
- [ ] `goral-front.png` — 산양 정면 — 줄 설 때
- [ ] `goral-back.png` — 산양 뒷모습 — 올라갈 때
- [ ] `marten-front.png` — 담비 정면 — 줄 설 때
- [ ] `marten-back.png` — 담비 뒷모습 — 올라갈 때
- [ ] `mandarin-front.png` — 원앙 정면 — 줄 설 때
- [ ] `mandarin-back.png` — 원앙 뒷모습 — 올라갈 때
- [ ] `wolf-front.png` — 늑대 정면 — 줄 설 때
- [ ] `wolf-back.png` — 늑대 뒷모습 — 올라갈 때
- [ ] `egret-front.png` — 백로 정면 — 줄 설 때
- [ ] `egret-back.png` — 백로 뒷모습 — 올라갈 때
- [ ] `leopard-front.png` — 표범 정면 — 줄 설 때
- [ ] `leopard-back.png` — 표범 뒷모습 — 올라갈 때
- [ ] `muskdeer-front.png` — 사향노루 정면 — 줄 설 때
- [ ] `muskdeer-back.png` — 사향노루 뒷모습 — 올라갈 때
- [ ] `moonbear-front.png` — 반달곰 정면 — 줄 설 때
- [ ] `moonbear-back.png` — 반달곰 뒷모습 — 올라갈 때
- [ ] `haetae-front.png` — 해태 정면 — 줄 설 때
- [ ] `haetae-back.png` — 해태 뒷모습 — 올라갈 때

### 손님 카드 1단 (1~5성) — `godot/art/cards/` · 512×768 · **0장 남음**

**세로로 긴 초상.** 정면을 보고 서 있는 모습, 배경까지 그린 한 장.
뽑기에서 크게 뜨고 도감에 모인다 — 이 게임에서 제일 오래 들여다보는 그림이다.
테두리·등급 표시는 코드가 그린다. **짐승과 배경만** 그리면 된다.
파일 이름은 `<손님id>-1.png`. 6~10성은 `-2`, 11~15성은 `-3`, 16~20성은 `-4`.

전부 들어왔다. ✅

### 점장 카드 (가게를 열 때 뜨는 초상) — `godot/art/cards/` · 512×768 · **15장 남음**

가게를 되살리거나 승급하면 화면 가운데에 카드가 뜬다 — **그 초상이 여태 없었다.**
손님 카드와 같은 규격(세로 초상, 배경까지). 그 가게 점장 너구리가 제 일터에서
일하는 모습으로. 파일명은 `<가게id>-1.png`(첫 등급). 승급 등급(-2, -3)은 선택.

- [ ] `smith-1.png` — 대장간 점장 초상 — 무쇠를 두드려 연장을 만든다
- [ ] `brush-1.png` — 필방 점장 초상 — 붓과 먹을 다룬다
- [ ] `paper-1.png` — 지물포 점장 초상 — 닥나무를 떠서 종이를 만든다
- [ ] `pot-1.png` — 옹기점 점장 초상 — 흙을 빚어 항아리를 굽는다
- [ ] `herb-1.png` — 약재상 점장 초상 — 산에서 캔 것을 말리고 썬다
- [ ] `soup-1.png` — 국밥집 점장 초상 — 뜨끈한 국밥 한 그릇을 만다
- [ ] `gaekju-1.png` — 객주 점장 초상 — 길손을 재우고 술과 안주를 낸다
- [ ] `skewer-1.png` — 꼬치집 점장 초상 — 숯불에 꿰어 굽는다
- [ ] `ricecake-1.png` — 떡집 점장 초상 — 쌀을 쳐서 떡을 빚는다
- [ ] `butcher-1.png` — 푸줏간 점장 초상 — 뼈를 발라 고기를 판다
- [ ] `fish-1.png` — 어물전 점장 초상 — 바닷것을 절이고 말려 판다
- [ ] `cloth-1.png` — 포목전 점장 초상 — 베를 짜고 물들여 판다
- [ ] `hat-1.png` — 갓방 점장 초상 — 갓을 겯고 다듬어 판다
- [ ] `brass-1.png` — 유기전 점장 초상 — 놋쇠를 두드려 그릇을 만든다
- [ ] `lacquer-1.png` — 나전방 점장 초상 — 자개를 박아 세간을 꾸민다

### 점장 카드 승급판 (2·3등급) (선택) — `godot/art/cards/` · 512×768 · **30장 남음**

- [ ] `smith-2.png` — 대장간 — 참쇠 등급
- [ ] `smith-3.png` — 대장간 — 강철 등급
- [ ] `brush-2.png` — 필방 — 고운 등급
- [ ] `brush-3.png` — 필방 — 명품 등급
- [ ] `paper-2.png` — 지물포 — 고운 등급
- [ ] `paper-3.png` — 지물포 — 진상 등급
- [ ] `pot-2.png` — 옹기점 — 오지 등급
- [ ] `pot-3.png` — 옹기점 — 왕실 등급
- [ ] `herb-2.png` — 약재상 — 묵은 등급
- [ ] `herb-3.png` — 약재상 — 천년 등급
- [ ] `soup-2.png` — 국밥집 — 곰국 등급
- [ ] `soup-3.png` — 국밥집 — 용봉탕 등급
- [ ] `gaekju-2.png` — 객주 — 청 등급
- [ ] `gaekju-3.png` — 객주 — 법 등급
- [ ] `skewer-2.png` — 꼬치집 — 참숯 등급
- [ ] `skewer-3.png` — 꼬치집 — 백탄 등급
- [ ] `ricecake-2.png` — 떡집 — 찹쌀 등급
- [ ] `ricecake-3.png` — 떡집 — 진상 등급
- [ ] `butcher-2.png` — 푸줏간 — 상 등급
- [ ] `butcher-3.png` — 푸줏간 — 진상 등급
- [ ] `fish-2.png` — 어물전 — 말린 등급
- [ ] `fish-3.png` — 어물전 — 진상 등급
- [ ] `cloth-2.png` — 포목전 — 명주 등급
- [ ] `cloth-3.png` — 포목전 — 비단 등급
- [ ] `hat-2.png` — 갓방 — 중 등급
- [ ] `hat-3.png` — 갓방 — 진상 등급
- [ ] `brass-2.png` — 유기전 — 상 등급
- [ ] `brass-3.png` — 유기전 — 왕실 등급
- [ ] `lacquer-2.png` — 나전방 — 진주 등급
- [ ] `lacquer-3.png` — 나전방 — 왕실 등급

### 계산대 (선택) — `godot/art/counters/` · 168×144 · **15장 남음**

점장이 서서 파는 곳. **정면치기 한 장이면 된다** — 마당 문 방향에 따라
코드가 좌우로 뒤집는다. 좌우 비대칭 소품(주판·엽전함)을 올려도 뒤집혀
어색하지 않게, 되도록 가운데 배치로. 없으면 나무 상자를 그린다.
★ **방향 약속**: 손님이 받아 가는 넓은 면이 그림의 **오른쪽 아래**를
보게 그린다(길이 그쪽이다). 왼쪽 골목을 보는 가게는 코드가 뒤집는다 —
지금 임시 상자가 어색해 보이는 건 방향이 없어서다(유저가 필방·옹기점·
약재상에서 잡았다). 그림이 오면 이 약속대로 방향이 생긴다.

- [ ] `smith.png` — 대장간 계산대
- [ ] `brush.png` — 필방 계산대
- [ ] `paper.png` — 지물포 계산대
- [ ] `pot.png` — 옹기점 계산대
- [ ] `herb.png` — 약재상 계산대
- [ ] `soup.png` — 국밥집 계산대
- [ ] `gaekju.png` — 객주 계산대
- [ ] `skewer.png` — 꼬치집 계산대
- [ ] `ricecake.png` — 떡집 계산대
- [ ] `butcher.png` — 푸줏간 계산대
- [ ] `fish.png` — 어물전 계산대
- [ ] `cloth.png` — 포목전 계산대
- [ ] `hat.png` — 갓방 계산대
- [ ] `brass.png` — 유기전 계산대
- [ ] `lacquer.png` — 나전방 계산대

### 가마·풀무 — `godot/art/kilns/` · 168×192 · **15장 남음**

마당 뒤쪽의 만드는 기계 — 대장간은 풀무·화덕, 옹기점은 가마, 국밥집은 가마솥.
승급 그림(`smith-1.png`·`smith-2.png`)을 넣으면 등급 따라 갈아입는다(선택).
없으면 코드가 등급 따라 키우고 불을 세게 피운다.

- [ ] `smith.png` — 대장간 — 무쇠를 두드려 연장을 만든다
- [ ] `brush.png` — 필방 — 붓과 먹을 다룬다
- [ ] `paper.png` — 지물포 — 닥나무를 떠서 종이를 만든다
- [ ] `pot.png` — 옹기점 — 흙을 빚어 항아리를 굽는다
- [ ] `herb.png` — 약재상 — 산에서 캔 것을 말리고 썬다
- [ ] `soup.png` — 국밥집 — 뜨끈한 국밥 한 그릇을 만다
- [ ] `gaekju.png` — 객주 — 길손을 재우고 술과 안주를 낸다
- [ ] `skewer.png` — 꼬치집 — 숯불에 꿰어 굽는다
- [ ] `ricecake.png` — 떡집 — 쌀을 쳐서 떡을 빚는다
- [ ] `butcher.png` — 푸줏간 — 뼈를 발라 고기를 판다
- [ ] `fish.png` — 어물전 — 바닷것을 절이고 말려 판다
- [ ] `cloth.png` — 포목전 — 베를 짜고 물들여 판다
- [ ] `hat.png` — 갓방 — 갓을 겯고 다듬어 판다
- [ ] `brass.png` — 유기전 — 놋쇠를 두드려 그릇을 만든다
- [ ] `lacquer.png` — 나전방 — 자개를 박아 세간을 꾸민다

### 손님 카드 2~4단 (6~20성) (선택) — `godot/art/cards/` · 512×768 · **0장 남음**

**나중에.** 없으면 1단 그림을 계속 쓴다.
같은 짐승이 성이 오를수록 차림이 좋아지는 식으로 — 옷·장신구·배경이 달라진다.

전부 들어왔다. ✅

### 나쁜 놈 — `godot/art/pests/` · 128×128 · **0장 남음**

전부 들어왔다. ✅

### 매대(좌판) — `godot/art/stalls/` · 192×176 · **5장 남음**

가게마다 **다르게 생겨야 한다.** 지금은 다섯 가게가 똑같은 나무 상자다.
그 가게에서 무엇을 파는지가 좌판에서 먼저 읽혀야 한다 —
대장간은 모루·쇠받침, 필방은 낮은 서안, 지물포는 종이 두루마리 선반,
옹기점은 흙바닥 좌판, 약재상은 약재 서랍장 같은 식으로.
**물건 그림이 이 위에 얹힌다** — 가운데 위쪽을 비워 둘 것.
승급 그림(`smith-1.png`)도 넣을 수 있다. 없으면 기본 것을 쓴다.
★ **↙ 한 장만 그리면 된다** — ↘쪽은 코드가 거울로 뒤집는다. 다만 거울은
빛 방향(왼쪽 위 광원)까지 뒤집으므로, 비대칭이 심해 어색한 가게만
`<가게id>-r.png`(↘판)를 따로 주면 그걸 쓴다. 되도록 좌우 대칭에 가깝게
구성하면 ↘판 없이도 자연스럽다.

- [ ] `fish.png` — 어물전 — 바닷것을 절이고 말려 판다
- [ ] `cloth.png` — 포목전 — 베를 짜고 물들여 판다
- [ ] `hat.png` — 갓방 — 갓을 겯고 다듬어 판다
- [ ] `brass.png` — 유기전 — 놋쇠를 두드려 그릇을 만든다
- [ ] `lacquer.png` — 나전방 — 자개를 박아 세간을 꾸민다

### 뽑기와 룰렛 판 — `godot/art/ui/` · 아래 참고 · **2장 남음**

움직임은 `MOTION.md`에 적어 뒀다.
· `back.png` 512×768 — 카드 뒷면. 뒤집기 연출에 쓴다. 등급 빛은 코드가 얹는다
· `wheel.png` 512×512 — 룰렛 원판. **열두 칸**이 그려진 바퀴. 칸 안 글자는 코드가 얹는다
· `needle.png` 64×96 — 룰렛 바늘. 위에서 아래를 가리킨다

- [ ] `coin.png` — 엽전 64×64 — 계산대 위·팔린 표에 쓴다 (상평통보처럼 가운데 네모 구멍)
- [ ] `gem.png` — 젬 64×64 — 💎 이모지 자리를 물려받는다

### 프로필 초상 (선택) — `godot/art/portraits/` · 256×256 · **31장 남음**

머리띠(위 표시줄) 왼쪽의 동그란 얼굴. 규격 네 줄이면 된다:
1. **동그란 틀 안에** 그린다 — 원 밖은 투명. 코드가 네모로 자르지 않는다.
2. **어깨까지 상반신**, 살짝 위에서 본 정면(3/4). 얼굴이 원의 7~8할.
3. 배경은 투명. 테두리 띠(칭호 색)와 바탕은 코드가 두른다.
4. 표정은 그 짐승의 성격대로(BESTIARY.md 참고) — 호랑이는 위엄, 토끼는 촐랑.
카드(512×768)와 같은 그림체·팔레트다. 카드가 이미 있는 손님은 그 얼굴을
동그란 틀에 다시 앉히면 된다 — 새로 창작할 필요 없다.

- [ ] `raccoon.png` — 기본 얼굴 — 주인공 너구리
- [ ] `rabbit.png` — 토끼 — 뽑으면 고를 수 있다
- [ ] `magpie.png` — 까치 — 뽑으면 고를 수 있다
- [ ] `squirrel.png` — 다람쥐 — 뽑으면 고를 수 있다
- [ ] `badger.png` — 오소리 — 뽑으면 고를 수 있다
- [ ] `fox.png` — 여우 — 뽑으면 고를 수 있다
- [ ] `deer.png` — 사슴 — 뽑으면 고를 수 있다
- [ ] `boar.png` — 멧돼지 — 뽑으면 고를 수 있다
- [ ] `bear.png` — 곰 — 뽑으면 고를 수 있다
- [ ] `turtle.png` — 거북 — 뽑으면 고를 수 있다
- [ ] `crane.png` — 두루미 — 뽑으면 고를 수 있다
- [ ] `ox.png` — 소 — 뽑으면 고를 수 있다
- [ ] `tiger.png` — 호랑이 — 뽑으면 고를 수 있다
- [ ] `sparrow.png` — 참새 — 뽑으면 고를 수 있다
- [ ] `frog.png` — 개구리 — 뽑으면 고를 수 있다
- [ ] `mole.png` — 두더지 — 뽑으면 고를 수 있다
- [ ] `hedgehog.png` — 고슴도치 — 뽑으면 고를 수 있다
- [ ] `duck.png` — 오리 — 뽑으면 고를 수 있다
- [ ] `otter.png` — 수달 — 뽑으면 고를 수 있다
- [ ] `roe.png` — 노루 — 뽑으면 고를 수 있다
- [ ] `weasel.png` — 족제비 — 뽑으면 고를 수 있다
- [ ] `wildcat.png` — 살쾡이 — 뽑으면 고를 수 있다
- [ ] `goral.png` — 산양 — 뽑으면 고를 수 있다
- [ ] `marten.png` — 담비 — 뽑으면 고를 수 있다
- [ ] `mandarin.png` — 원앙 — 뽑으면 고를 수 있다
- [ ] `wolf.png` — 늑대 — 뽑으면 고를 수 있다
- [ ] `egret.png` — 백로 — 뽑으면 고를 수 있다
- [ ] `leopard.png` — 표범 — 뽑으면 고를 수 있다
- [ ] `muskdeer.png` — 사향노루 — 뽑으면 고를 수 있다
- [ ] `moonbear.png` — 반달곰 — 뽑으면 고를 수 있다
- [ ] `haetae.png` — 해태 — 뽑으면 고를 수 있다

### 가게별 점장 (완성형 · 새 계약) — `godot/art/clerks/` · 144×144 · **30장 남음**

가게마다 점장을 **통으로** 그린다 — 몸·옷·연장·꼬리를 한 장에.
그림체 기준은 통통 너구리(HERO-A 턴어라운드, docs/art/generated) — 그 몸에
그 가게의 옷과 연장을 입힌 모습이다. 방향은 둘만 그린다:
· `<가게>-side.png` — **오른쪽**을 향한 훼이크 측면(정면에 가까운 몸+뻗은 팔).
  일하기·계산·서 있기 전부 이 한 장이다. 왼쪽은 코드가 뒤집는다
· `<가게>-back.png` — 뒷모습. 화면 위로 걸어갈 때
**정면은 안 그린다**(서 있을 땐 마지막 방향 그대로 멈춘다 — 유저 결정).
선택: `<가게>-sleep.png`(조는 모습 — 없으면 코드가 옆모습에 💤를 얹는다),
승급판 `<가게>-side-1.png`(2등급)·`-2`(3등급) 등 — 없으면 1등급 것을 쓴다.
발끝은 아래 변에, 몸은 가로 정중앙에. 그림자·움직임 프레임 금지(코드 몫).
옛 make/sell 20장은 새 그림이 올 때까지만 돈다 — 추가 제작 금지.

- [ ] `smith-side.png` — 대장간 점장 — 옆(일·계산·서기 전부)
- [ ] `smith-back.png` — 대장간 점장 — 뒷모습(위로 걸을 때)
- [ ] `brush-side.png` — 필방 점장 — 옆(일·계산·서기 전부)
- [ ] `brush-back.png` — 필방 점장 — 뒷모습(위로 걸을 때)
- [ ] `paper-side.png` — 지물포 점장 — 옆(일·계산·서기 전부)
- [ ] `paper-back.png` — 지물포 점장 — 뒷모습(위로 걸을 때)
- [ ] `pot-side.png` — 옹기점 점장 — 옆(일·계산·서기 전부)
- [ ] `pot-back.png` — 옹기점 점장 — 뒷모습(위로 걸을 때)
- [ ] `herb-side.png` — 약재상 점장 — 옆(일·계산·서기 전부)
- [ ] `herb-back.png` — 약재상 점장 — 뒷모습(위로 걸을 때)
- [ ] `soup-side.png` — 국밥집 점장 — 옆(일·계산·서기 전부)
- [ ] `soup-back.png` — 국밥집 점장 — 뒷모습(위로 걸을 때)
- [ ] `gaekju-side.png` — 객주 점장 — 옆(일·계산·서기 전부)
- [ ] `gaekju-back.png` — 객주 점장 — 뒷모습(위로 걸을 때)
- [ ] `skewer-side.png` — 꼬치집 점장 — 옆(일·계산·서기 전부)
- [ ] `skewer-back.png` — 꼬치집 점장 — 뒷모습(위로 걸을 때)
- [ ] `ricecake-side.png` — 떡집 점장 — 옆(일·계산·서기 전부)
- [ ] `ricecake-back.png` — 떡집 점장 — 뒷모습(위로 걸을 때)
- [ ] `butcher-side.png` — 푸줏간 점장 — 옆(일·계산·서기 전부)
- [ ] `butcher-back.png` — 푸줏간 점장 — 뒷모습(위로 걸을 때)
- [ ] `fish-side.png` — 어물전 점장 — 옆(일·계산·서기 전부)
- [ ] `fish-back.png` — 어물전 점장 — 뒷모습(위로 걸을 때)
- [ ] `cloth-side.png` — 포목전 점장 — 옆(일·계산·서기 전부)
- [ ] `cloth-back.png` — 포목전 점장 — 뒷모습(위로 걸을 때)
- [ ] `hat-side.png` — 갓방 점장 — 옆(일·계산·서기 전부)
- [ ] `hat-back.png` — 갓방 점장 — 뒷모습(위로 걸을 때)
- [ ] `brass-side.png` — 유기전 점장 — 옆(일·계산·서기 전부)
- [ ] `brass-back.png` — 유기전 점장 — 뒷모습(위로 걸을 때)
- [ ] `lacquer-side.png` — 나전방 점장 — 옆(일·계산·서기 전부)
- [ ] `lacquer-back.png` — 나전방 점장 — 뒷모습(위로 걸을 때)

<!-- 목록끝 -->

## 넣는 법

1. `godot/art/<갈래>/<이름>.png` 로 넣는다 (위 목록의 이름 그대로)
2. `godot --headless --path godot --import` 한 번 (에디터를 켜 뒀으면 저절로 한다)
3. 끝. **하나만 넣어도 그 자리만 바뀐다** — 없는 자리는 여태처럼 도형과 이모지다
4. 목록 갱신: `node tools/art.mjs --write`
