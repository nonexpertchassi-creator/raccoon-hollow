...
```mermaid
flowchart TD
    A["게임을 켠다"] --> B["대장간부터 세운다<br/>작업대 1단 — 호미"]
    B --> C["손님이 오고 돈이 오른다"]
    C --> D{"뭘 해야 하지?"}
    D --> E["아무 설명이 없다"]
...
flowchart TD
    ...
    D --> E["작업대를 누른다<br/>→ 호미가 놓인다"]
    E --> F["첫 손님이 온다<br/>주문표 🪓1"]
    F --> G["돈이 들어온다<br/>→ 작업대를 눌러 강화"]
...
물건 강화 · 작업대 한 단 더 · 가게 열기 · 승급 · 일꾼 · 젬 강화 · 의뢰 · 기간제 이벤트 ·
...
뽑기·손님 서른·가게 열이 한꺼번에 들어왔다. **규칙 하나를 바꾸면
...
- 계절은 **실제 달력**으로(저장본이 이미 실제 시간을 적어 둔다 — `wall`) 4달에 한 번씩
...
**언제:** 낼 때 필요한 다섯(처음 3분·의뢰·소리·설정·폰 검증)이
