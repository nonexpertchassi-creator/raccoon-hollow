...
const SIZE := {
	"hero": Vector2(72, 72),
	# ★ "clerks"는 이제 **폴더 이름이 아니라 크기 이름**이다(2026-08-28).
	#   가게별 일꾼 그림 18장을 지웠지만, 마당의 일꾼을 그릴 때 쓰는
	#   크기 이름으로 그대로 남는다. 지우면 일꾼이 엉뚱한 크기로 앉는다.
	"clerks": Vector2(72, 72),
	# 작업대 0.85 × 0.85 × 0.70 → 82 × 74. (96×88에서 줄인 값 — 한 칸에서 위로
	# 쏠려 보이고 상품과 겹쳤다. 작업대는 상품 받침이지 주인공이 아니다.)
	# ★ "stalls"는 이제 **그림 폴더 이름**이지 규칙의 이름이 아니다(2026-08-29).
	#   규칙 쪽에서는 매대를 **작업대**로 갈아 끼웠는데(sim.bench_*), 그림
	#   폴더까지 옮기면 그림 그리는 쪽이 받아 둔 파일 이름이 통째로 어긋난다.
	#   그래서 폴더 이름만 옛말로 남겨 둔다 — clerks 때와 같은 처리다.
