...
const SIZE := {
	...
	"staff": Vector2(72, 72),
	"stalls": Vector2(96, 88),       # 매대(좌판) — 한 칸을 거의 채운다
...
static var _pad: Dictionary = {}

## 그림 아래쪽 투명 여백이 전체 높이의 몇 할인가. **한 번 재서 기억한다.**
## 발끝 기준으로 놓는 그림이 밑에 여백을 달고 오면 그만큼 공중에 뜨고,
## 코드가 깐 그림자와 어긋난다(2026-08-25, 유저가 잡았다). 그림마다 여백이
## 달라서(점장 9%, 손님 0%) 사람이 맞추는 건 끝이 없다 — 재는 게 맞다.
static func foot_pad(t: Texture2D) -> float:
