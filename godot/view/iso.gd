...
## ── 방향 넷 ──
## 이 화면은 비스듬히 내려다본 격자라, 무엇이든 **네 대각선**으로만 움직인다.
## 격자의 +x는 화면에서 ↘, +y는 ↙, −x는 ↖, −y는 ↗이다.
##
## ★ 2026-08-28에 **뒤집기를 그만뒀다**(유저: "↗ ↖ ↘ ↙ 이 방향 다 그리자").
##   옛날에는 오른쪽 한 장을 그려 왼쪽은 코드가 거울로 뒤집었다. 그런데 거울은
##   **빛 방향까지 뒤집는다** — 왼쪽 위에서 오는 볕이 오른쪽 위에서 오게 되고,
##   비대칭 소품(망치·앞치마 매듭)이 반대 손으로 간다. 한 장을 아끼려고
##   그때마다 "왜 어색하지"를 되풀이했다. 네 장을 다 그리면 그 질문이 사라진다.
const DIRS: Array[String] = ["ne", "nw", "se", "sw"]

## 화면 위에서의 움직임(또는 바라보는 쪽)을 네 방향 중 하나로 바꾼다.
static func dir4(d: Vector2) -> String:
	if d.y < 0.0:
		return "ne" if d.x >= 0.0 else "nw"
	return "se" if d.x >= 0.0 else "sw"

## 격자 칸에서 칸으로 볼 때의 방향.
static func dir4_tile(from_t: Vector2i, to_t: Vector2i) -> String:
	return dir4(w(to_t.x, to_t.y) - w(from_t.x, from_t.y))
