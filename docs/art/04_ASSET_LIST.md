...
> 최신 게임은 Godot으로 전환됐고 자동 주문 목록은 `필수 74장 + 선택 10장`을 가리킨다.
> 현재 필수 상태는 `확정 0 / 초안 11 / 없음 63`이다. 파일 존재만으로 확정하지 않는다.
> 직원 4등급의 작업·수면 8장은 새 규격 초안으로 모두 들어왔고, 최신 주문서에는 촌장 1장과 좌판 5장이 추가됐다.
