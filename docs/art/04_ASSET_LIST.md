...
> 최신 게임은 Godot으로 전환됐고 자동 주문 목록은 `필수 125장 + 선택 100장`을 가리킨다.
> 확인 기준은 Claude 브랜치 `e49395b`, 현재 필수 상태는 `확정 0 / 초안 31 / 없음 94`이다.
> 필수에는 손님 필드 30장·카드 1단 30장·뽑기/룰렛 UI 3장이 포함되고, 선택에는 성장 카드 90장과 전용 점장 10장이 포함된다.
> 직원 8장, 점장·촌장 6장, 손님 17장(토끼 구규격 포함)은 초안으로 들어왔지만 파일 존재만으로 확정하지 않는다.
