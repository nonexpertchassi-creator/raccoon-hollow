...
> 브라우저에서 실제 이미지와 필수 125장 + 선택 100장 카드를 함께 보려면
...
- 필수 이미지: 자동 목록 기준 `125장`
- 선택 이미지: 성장 카드 `90장` + 가게별 점장 `10장`
- 필수 상태: `확정 0장 / 초안 21장 / 없음 104장`
- 전체 관리 대상: `225장` (`초안 21장 / 없음 204장`)

### 중요한 현재 상태

최신 브랜치에는 30종 손님, 카드 뽑기, 룰렛과 움직임 계약이 추가됐다. 기존 점장
`make/sell`과 토끼는 실제 Godot 화면에 표시되지만 새 제작 규격의 절반 크기인 구규격이다.
직원 8장, 점장 걷기·수면·촌장 4장, 까치를 포함한 손님 6장은 새 규격으로 생성했다. 스물한 장 모두
...
| 손님 | 토끼·까치·다람쥐·오소리·여우·사슴·멧돼지 초안 7장, 없음 23장 | 필드용 30종. 나머지는 도형·이모지 |
| 손님 카드 | 없음 30장 + 선택 90장 | 1단이 필수, 2~4단은 선택 성장 그림 |
| 물건 | 없음 40장 | 이모지로 대체 |
| 나쁜 놈 | 없음 3장 | 이모지로 대체 |
| 좌판 | 없음 5장 | 가게별 PNG가 없으면 공통 나무 상자로 대체 |
| 뽑기·룰렛 | 없음 3장 | 카드 뒷면, 12칸 원판, 바늘 |
...
최신 `ASSETS.md`, `MOTION.md`, `tools/art.mjs` 기준 필수 총수는 `125장`이다.
손님 필드 30장과 카드 1단 30장, 카드 뒷면·룰렛 원판·바늘이 추가됐다.
성장 카드 90장과 가게별 점장 10장은 폴백이 있어 선택 작업으로 분리됐다.

| 분류 | 전체 | 확정 | 초안 | 없음 | 제작 규격 → 화면 표시 |
|---|---:|---:|---:|---:|---|
| 공용 점장·촌장 | 6 | 0 | 6 | 0 | 144×144 → 72×72 |
| 직원 | 8 | 0 | 8 | 0 | 144×144 → 72×72 |
| 물건 | 40 | 0 | 0 | 40 | 128×224 → 64×112 |
| 손님 필드 | 30 | 0 | 7 | 23 | 128×128 → 64×64 |
| 손님 카드 1단 | 30 | 0 | 0 | 30 | 512×768 → 카드 화면 |
| 쥐·까마귀·삽살개 | 3 | 0 | 0 | 3 | 128×128 → 64×64 |
| 가게별 좌판 | 5 | 0 | 0 | 5 | 192×176 → 96×88 |
| 카드 뒷면·룰렛 | 3 | 0 | 0 | 3 | 512×768 / 512×512 / 64×96 |
| **필수 합계** | **125** | **0** | **21** | **104** | |
| 손님 성장 카드 (선택) | 90 | 0 | 0 | 90 | 512×768 |
| 가게별 점장 (선택) | 10 | 0 | 0 | 10 | 144×144 → 72×72 |
| **전체 관리 대상** | **225** | **0** | **21** | **204** | |

제작 우선순위는 최신 주문서대로 `손님 필드 30 → 카드 1단 30 → 점장 → 직원 → 물건 40 → 좌판·나쁜 놈`이다.
점장과 직원은 이미 모두 초안이므로 지금은 손님 필드부터 채운다. 성장 카드와 전용 점장은 필수 작업 뒤로 둔다.
