...
| **전체 관리 대상** | **280** | **0** | **190** | **90** | |

제작 우선순위는 최신 주문서대로 `손님 필드 30 → 카드 1단 30 → 점장 → 직원 → 물건 80 → 좌판·나쁜 놈`이다.
손님 필드 30종, 손님 카드 1단 30종, 점장, 직원, 물건 80종, 좌판 10종, 상호작용 3종과 룰렛 UI는 모두 초안이다. 필수 170장과 선택 전용 점장 20장을 모두 채웠다. 남은 선택 작업은 성장 카드 90장이다.
