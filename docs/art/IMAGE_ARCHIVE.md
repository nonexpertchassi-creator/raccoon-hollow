...
| 선택 전용 점장 | 초안 15장, 없음 5장 | 기존 다섯 가게 두 포즈 + 국밥집·주막 두 포즈 + 꼬치집 제작, 나머지는 공용 점장 폴백 |
...
| 가게별 점장 (선택) | 20 | 0 | 15 | 5 | 144×144 → 72×72 |
| **전체 관리 대상** | **280** | **0** | **185** | **95** | |

제작 우선순위는 최신 주문서대로 `손님 필드 30 → 카드 1단 30 → 점장 → 직원 → 물건 80 → 좌판·나쁜 놈`이다.
손님 필드 30종, 손님 카드 1단 30종, 점장, 직원, 물건 80종, 좌판 10종, 상호작용 3종과 룰렛 UI는 모두 초안이다. 필수 170장은 모두 채워졌고 선택 전용 점장은 15장을 채웠다. 남은 전용 점장 5장 뒤에 성장 카드 90장을 둔다.
