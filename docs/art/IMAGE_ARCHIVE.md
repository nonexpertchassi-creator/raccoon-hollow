...
> 브라우저에서 실제 이미지와 필수 74장 + 선택 10장 카드를 함께 보려면
...
- 필수 이미지: 자동 목록 기준 `74장`
- 선택 이미지: 가게별 점장 `10장`
- 필수 상태: `확정 0장 / 초안 11장 / 없음 63장`
- 전체 관리 대상: `84장`

### 중요한 현재 상태

최신 브랜치에는 `godot/view/art.gd` 로더가 생겼고 기존 PNG도 `godot/art/`로
이동했다. 기존 점장 2장과 토끼는 실제 Godot 화면에 표시되지만 새 제작 규격의
절반 크기인 구규격 파일이다. 직원 4등급의 작업·수면 8장은 새 144×144 규격으로
생성했지만 아직 실제 화면 검수와 사용자 승인을 거치지 않았다. 열한 장 모두 `초안`으로 분류한다.

| 구분 | 상태 | 최신 Godot 처리 |
|---|---|---|
| 점장 | 초안 2장 | PNG 표시, 없는 자세는 공용 자세·도형으로 대체 |
| 직원 | 초안 8장 | 머리띠·초립·패랭이·백립의 작업·수면이 모두 연결·화면 검수 대기 |
| 손님 | 토끼 초안 1장, 없음 11장 | 토끼는 PNG, 나머지는 도형·이모지 |
| 물건 | 없음 40장 | 이모지로 대체 |
| 나쁜 놈 | 없음 3장 | 이모지로 대체 |
...
최신 `ASSETS.md`와 `tools/art.mjs` 기준 필수 총수는 `74장`이다. 촌장 1장과
가게별 좌판 5장이 추가됐고, 가게별 점장 10장은 공용 점장으로 대체할 수 있어 선택 작업으로 분리됐다.

| 분류 | 전체 | 확정 | 초안 | 없음 | 제작 규격 → 화면 표시 |
|---|---:|---:|---:|---:|---|
| 공용 점장·촌장 | 6 | 0 | 2 | 4 | 144×144 → 72×72 |
| 직원 | 8 | 0 | 8 | 0 | 144×144 → 72×72 |
| 물건 | 40 | 0 | 0 | 40 | 128×224 → 64×112 |
| 손님 | 12 | 0 | 1 | 11 | 128×128 → 64×64 |
...
제작 우선순위는 `점장·촌장 6 → 직원 8 → 손님 12 → 물건 40 → 나쁜 놈 3 → 좌판 5`를 기본으로 하되,
Godot 화면에서 먼저 필요한 좌판은 클로드 요청에 따라 앞당길 수 있다.
가게별 점장 10장은 필수 작업을 마친 뒤 제작한다.
