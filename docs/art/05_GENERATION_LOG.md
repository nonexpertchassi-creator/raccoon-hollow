...
- D 재설계 원화: `CLERK-SMITH-TYPE-D-GROWTH-DIAGONALS-V0.1-P07.png`
...
- 실제 런타임에 필요한 `↘ side`와 `↗ back` 두 방향만 D 원화에 남겼다. 최초 생성된 뒤 방향이 `↖`으로 읽혀 두 번째 열을 정확히 반전하여 `↗`으로 수정했다.
- A~C의 네 방향 판은 검토 원화로 보존하지만 런타임 파생은 동일하게 `↘`·`↗`만 사용한다.

### 판정

- 24/24장 모두 `144×144 RGBA`, 발끝 `y=143`, 가로 중심 오차 1px 이내를 통과했다.
- D 뒤 방향은 얼굴 끝이 화면 오른쪽, 꼬리가 화면 왼쪽에 있어 `↗`으로 읽힌다.
