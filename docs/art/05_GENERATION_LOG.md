...
### LOG-CARD-023 — 손님 성장 카드 4단 다섯 번째 5종 P01

- 날짜: `2026-08-23`
- Prompt ID: `GUEST-CARDS-TIER4-BATCH-05-P01`
- Prompt source: [prompts/GUEST-CARDS-TIER4-BATCH-05-P01.md](./prompts/GUEST-CARDS-TIER4-BATCH-05-P01.md)
- 게임용 초안: `wildcat-4.png`, `goral-4.png`, `marten-4.png`, `mandarin-4.png`, `wolf-4.png`
- QA: [qa/guest-cards-tier4-batch-05.png](./qa/guest-cards-tier4-batch-05.png)
- 상태: `Generated` / 그림 장부 `초안`

#### 관찰

- 다섯 장 모두 3단의 같은 개체·자세·계절과 황혼길·산길·단풍마당·연못·겨울길을 유지한다.
- 살쾡이·늑대는 열린 길과 고정 창호, 산양·담비·원앙은 연속된 길·담·물길로 4단을 설명한다.
- 털무늬와 원앙의 깃색이 먼저 읽히지만 산양 뒤의 계곡마을 지붕 밀도는 실제 카드 틀 검수가 필요하다.
- 다섯 런타임 파일은 정확한 `512×768`이다.

#### 판정

- 결과: `Generated`
- Why: 명암과 계절을 바꾸지 않아도 관리 상태만으로 4단이 읽히지만 산양 배경 밀도와 사용자 승인이 남았다.
- Hard fail 여부: 의상·직업 자세·추가 동물·프레임·문자·별·등급색·금장·이동식 등롱·마법 효과·워터마크 없음.
...
## LOG-MODULAR-001 · 점장 A~D 레이어와 대장간 장비·물건 3단

- 날짜: 2026-08-26
- Prompt ID: `SMITH-MODULAR-ASSETS-V0.1`
- 도구: built-in image generation/edit + background extraction
- 결과: `Generated`

### 관찰

- B·C·D는 승인된 A의 정면·훼이크 측면·뒷모습 가시 상자에 맞추고 무늬만 달리했다.
- A 포함 본체 네 타입의 발끝을 144×144 아래 변에 붙였다. 꼬리는 A 핀 크기를 복제했다.
- 장비는 본체 픽셀이 없는 앞치마+망치 스티커다. 앞치마를 손 아래로 내리고 망치는 길게 유지했다.
- 대장간 물건 7종은 무쇠·참쇠·강철 3단으로 만들었다. 얼굴·마법 효과 없이 재료와 마감만 성장한다.
- 모든 게임 파일은 RGBA이며 본체·꼬리·장비 144×144, 물건 128×224다.

### 판정

- Why: 스티커 인형놀이 구조와 작은 화면 실루엣은 성립했다. 실제 Godot 꼬리 위치와 장비 핀은 사용자 조정값이 확정된 뒤 최종 승인한다.
- Hard fail 여부: 셔츠·바지·신발·머리띠·라쿤 고리 꼬리·몸이 섞인 장비·판타지 보석·텍스트·불투명 배경 없음.

---

## LOG-SMITH-COMPLETE-001 · 대장간 일꾼 A~D 완성형 1~3단 대각선 시트

- 날짜: 2026-08-26
- Prompt ID: `CLERK-SMITH-TYPES-GROWTH-DIAGONALS-V0.1`
- 도구: built-in image generation
- 결과: `Generated / Draft`

### 관찰

- 분리형 본체·꼬리·장비 스티커 규칙을 중단하고 타입별 완성형 일꾼으로 전환했다. 최초 배치와 추가 채용을 구분하지 않는다.
- A~D 각 시트는 1단 황토색 면 앞치마, 2단 갈색 가죽 앞치마, 3단 멜빵형 가죽 앞치마를 한눈에 비교한다.
- 손에는 망치나 상품을 들리지 않았고 앞치마만으로 승급 차이를 만들었다.
- 정면·측면·뒷모습 대신 실제 이동축 `↙ ↘ ↖ ↗`을 목표로 구성했다.

### 판정

- Why: 앞치마 재료와 멜빵으로 1~3단의 성장은 읽히며 A~D 무늬도 유지됐다.
- 현재 상태: `초안`. 가까운 두 방향의 코·발·꼬리 위치가 충분히 반대로 읽히지 않는 칸이 있어 런타임 144×144 파생은 보류한다.
- Hard fail 여부: 손 도구·셔츠·바지·모자·신발·판타지 장식·과한 관절 없음.

### 다음 수정

- `↙`과 `↘`의 얼굴 각도, 앞치마 중심, 꼬리 위치를 명확히 반대로 만든다.
- 승인된 칸만 144×144 RGBA 독립 파일로 자르고 Godot 실제 크기에서 검수한다.

---

## LOG-SMITH-COMPLETE-002 · 대장간 일꾼 완성형 24장 런타임 분리

- 날짜: 2026-08-26
- 원본: `CLERK-SMITH-TYPE-A~D-GROWTH-DIAGONALS-V0.1-P01`
- 도구: Sharp 기반 반복 가능 분리 도구 `tools/slice-smith-clerks.cjs`
- 결과: `Generated / Draft`

### 관찰

- 승인 시트 네 장을 다시 그리지 않고 A~D 각 3단에서 오른쪽 기준 `↘`·`↗` 두 방향만 골라 총 24장을 만들었다.
- `↘`는 `side`, `↗`는 `back`이며 반대쪽 이동은 코드가 좌우 반전한다.
- 체크 배경은 셀 가장자리와 연결된 밝은 중립색만 제거하고, 캐릭터와 연결된 가장 큰 픽셀 덩어리만 보존했다.
- 모든 캐릭터에 하나의 공통 배율을 적용해 타입·단계마다 크기가 출렁이지 않게 했다.

### 판정

- 24/24장 모두 144×144 RGBA다.
- 발끝의 마지막 불투명 픽셀은 모두 y=143이며, 가로 중심 오차는 최대 0.5px다.
- 파일명은 `smith-<a~d>-<side|back>[-1|-2].png`이다.
- 현재 상태: `초안`. 최신 기능 로더 계약과 파일명이 일치하며 실제 마을 이동 화면에서 방향 판독을 검수한다.

---

## LOG-SMITH-COMPLETE-003 · 대장간 A~D 방향·귀·발 정렬 수정

- 날짜: 2026-08-26
- 수정 원화: A `P02`, B `P03`, C `P02`, D `P03`
- 런타임 미리보기: `generated/SMITH-CLERK-RUNTIME-24-PREVIEW-V0.2.png`
- 도구: built-in image generation/edit + Sharp 기반 런타임 분리
- 결과: `Revised / Draft`

### 관찰

- 가까운 두 열이 같은 방향처럼 보이던 문제를 고쳐 `↙`·`↘`로 분리했다.
- 두 번째 열은 코·눈·꼬리·발·앞치마 묶음이 첫 번째 열과 반대쪽에 놓인다.
- 귀 전체가 캔버스 안에 들어오고 발은 배 아래에서 같은 접지선을 사용하도록 신체 블루프린트에 맞췄다.
- 수정 원화에서 게임용 24장을 다시 파생했다.

### 판정

- 24/24장 모두 `144×144 RGBA`다.
- 발끝은 모두 `y=143`, 가로 중심 오차는 1px 이내이며 귀 위쪽 잘림이 없다.
- 현재 상태: `초안`. 실제 Godot 이동 화면에서 좌우 반전과 앞뒤 판독을 최종 확인한다.

---

## LOG-SMITH-COMPLETE-004 · A~C 차렷자세 확정과 D타입 2방향 재설계

- 날짜: 2026-08-26
- 확정 원화: A `P03`, B `P04`, C `P03`
- D 재설계 원화: `CLERK-SMITH-TYPE-D-GROWTH-DIAGONALS-V0.1-P08.png`
- 런타임 미리보기: `generated/SMITH-CLERK-RUNTIME-24-PREVIEW-V0.3.png`
- 도구: built-in image generation/edit + Sharp 기반 런타임 분리
- 결과: `Revised / Draft`

### 관찰

- A~C는 보행처럼 벌어진 발을 같은 크기·같은 접지선의 차렷자세로 교정했다.
- D는 방향마다 뒤집히던 귀 무늬와 점박이를 폐기했다. 연황색 귀, 단순 얼굴 마스크, 뒤쪽의 큰 등판 한 면, 단색 꼬리 끝만 사용한다.
- 실제 런타임에 필요한 `↘ side`와 `↗ back` 두 방향만 D 원화에 남겼다. 최초 생성된 두 열이 각각 반대 방향으로 읽혀 앞·뒤 열을 정확히 반전해 A~C와 방향을 통일했다.
- A~C의 네 방향 판은 검토 원화로 보존하지만 런타임 파생은 동일하게 `↘`·`↗`만 사용한다.

### 판정

- 24/24장 모두 `144×144 RGBA`, 발끝 `y=143`, 가로 중심 오차 1px 이내를 통과했다.
- D 앞·뒤 방향 모두 얼굴 끝이 화면 오른쪽, 꼬리가 화면 왼쪽에 있어 각각 `↘`·`↗`으로 읽힌다.
- 현재 상태: `초안`. Godot 이동 화면에서 좌우 반전만 최종 확인한다.

---

## LOG-SMITH-COMPLETE-005 · A~D 크기·시점·톤 전수 감사

- 날짜: 2026-08-26
- 감사 문서: `blueprints/SMITH-RACCOON-QA-AUDIT-2026-08-26.md`
- 영구 지침: `blueprints/CHARACTER-SPRITE-QA-CHECKLIST-V0.1.md`
- 결과: `Hold / Revision required`

### 발견

- D side는 110~111×137~138px, back은 104~105×139~140px로 A~C보다 크다.
- D 눈은 약 10×13px로 A의 약 7×10px보다 크고, 귀·선·밝기까지 다른 톤이다.
- B 눈은 A보다 약 5~6px 아래여서 머리 윗면이 많이 보이는 하이앵글처럼 읽힌다.
- A~C는 D가 결정한 공통 배율 때문에 블루프린트 목표보다 작아졌다.
- C 3단 side는 가장 큰 연결 덩어리만 남기는 파생 과정에서 꼬리가 누락됐다.

### 원인

- 생성 타입을 따로 확정했고 A~D를 한 판에서 겹쳐 보지 않았다.
- 방향·차렷자세·눈·시점보다 투명 배경·중앙·접지선 기계 검사를 먼저 통과 기준으로 삼았다.
- 가장 큰 캐릭터 기준 공통 배율과 단일 연결 덩어리 보존 방식이 캐릭터 파생에 맞지 않았다.
...
- 날짜: 2026-08-27
- 게임 파일: `godot/art/kilns/smith.png`
- 규격: `168×192 RGBA`, 실제 표시 `84×96`
- 도구: built-in image generation + `tools/fit-transparent-prop.cjs`

### 판정

- 짧은 숯 화덕과 붙박이 나무 풀무가 한 덩어리로 읽히며 좌판의 모루와 기능이 겹치지 않는다.
- 거의 정면인 3/4 시점이라 마당 방향이 바뀌어도 별도 반전 원화가 필요 없다.
- 건물·지붕·인물·망치·바구니·바닥·연기 덩어리를 제외해 84×96에서도 화덕 입구와 풀무가 먼저 읽힌다.
- 서양 성채형 용광로와 판타지 광효과 없이 한국 대장간 참고안의 숯 화덕 구조를 단순화했다.
