...
const GROUPS = [
  ...
  { dir: 'hero-body', size: '144×144', title: '일꾼 너구리 A~D (완성 캐릭터)',
    note: '가게 구분 없는 **일꾼 너구리** 네 무늬 — 같은 몸 윤곽, 다른 털(A~D).\n' +
          '저장본이 너구리마다 무늬를 무작위 배정한다("쌍둥이 말고 랜덤" — 유저).\n' +
          '방향 둘: `-side`(오른쪽 훼이크 측면 — 일·계산·서기 전부, 왼쪽은 코드\n' +
          '뒤집기) · `-back`(뒷모습). 정면은 안 그린다. **꼬리까지 몸에 그려\n' +
          '넣는다**(흔들기 없음). 발끝은 아래 변, 몸은 가로 정중앙, 그림자 금지.\n' +
          'a 두 장은 이미 들어왔다 — 같은 그림체(통통 너구리)로 나머지를.',
    rows: ['a', 'b', 'c', 'd'].flatMap((f) => [
      { id: `${f}-side`, why: `무늬 ${f.toUpperCase()} — 옆(일·계산·서기 전부)` },
      { id: `${f}-back`, why: `무늬 ${f.toUpperCase()} — 뒷모습(위로 걸을 때)` },
    ]) },
];
