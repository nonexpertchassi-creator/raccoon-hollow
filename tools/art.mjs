...
const GROUPS = [
  ...
  /* 첫 만화 — 게임을 처음 켤 때 한 번만 보여준다(건너뛸 수 있다).
   * ★ 이 만화가 **이 게임이 무엇인지 말하는 유일한 자리**다. 여기가 비면
   *   유저는 "숫자가 올라가는 것" 말고는 아무것도 못 받는다(2026-08-28). */
  { dir: 'intro', size: '512×320', title: '첫 만화',
  ...
  /* ★ **가게 앞치마·연장 30장을 뺐다**(2026-08-29, 유저: *"앞치마 30장 저것도
   *   빼라 했었던 거 같은데…"*).
   *
   *   찾아보니 **주문서가 스스로 모순돼 있었다.** 바로 위 hero-body 설명에는
   *   *"앞치마·연장은 안 그린다 — 가게 정체성은 마당과 손에 든 물건이 낸다"*라고
   *   적혀 있는데, 그 아래에서 30장을 주문하고 있었다. 한 사실이 두 곳에 있고
   *   복사본을 지켜 주는 것이 없으면 이렇게 된다(2026-08-28에 진단한 병).
   *
   *   코드(`village.gd` `_clerk_layers` · `card.gd`)는 아직 gear를 찾는다.
   *   찾다가 없으면 그냥 안 그린다 — 그러니 코드는 안 건드린다. 나중에
   *   앞치마를 다시 그리기로 하면 그날 주문서에 되살리면 된다.
   */
  /* ★ **가게 전용 일꾼(clerks/) 18장을 지웠다**(2026-08-28, 유저: "옛 동물 다
   *   지우라 했는데 너구리가 왜 보이냐").
   *
   *   그냥 남은 게 아니라 **새 그림을 가로막고 있었다.** 화면이 그림을 찾을 때
   *   이 갈래를 제일 먼저 봤기 때문에, 무늬 A를 아무리 잘 그려 넣어도
   *   대장간·필방·객주 같은 아홉 가게는 영영 옛 그림으로 나왔을 것이다.
   *   옛 계약(옆모습 한 장 + 거울 뒤집기)이라 4방향과 맞지도 않았다.
   *   꼬리 8장·대신 나오는 너구리 5장과 똑같은 종류다 —
   *   **옛것을 그물로 남기면 새것을 가린다.** 이번 주에 세 번째다. */
];
