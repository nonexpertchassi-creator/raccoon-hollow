...
const GROUPS = [
  ...
  { dir: 'staff', size: '144×144', title: '직원 너구리',
    note: '**공용 몸 한 벌이다 — 점장 무늬(a~d) 배정과 무관하다.** 크기는 점장과\n' +
          '같다(크기로 가르면 덜 자란 너구리처럼 보인다). 가르는 것은 **머리에 쓴 것**뿐.\n' +
  ...
  { dir: 'clerks', size: '144×144', title: '가게별 점장 (완성형 · 새 계약)',
    note: '가게마다 점장을 **통으로** 그린다 — 몸·옷·연장·꼬리를 한 장에.\n' +
          '그림체 기준은 통통 너구리(HERO-A 턴어라운드, docs/art/generated) — 그 몸에\n' +
          '그 가게의 옷과 연장을 입힌 모습이다. 방향은 둘만 그린다:\n' +
          '· `<가게>-side.png` — **오른쪽**을 향한 훼이크 측면(정면에 가까운 몸+뻗은 팔).\n' +
          '  일하기·계산·서 있기 전부 이 한 장이다. 왼쪽은 코드가 뒤집는다\n' +
          '· `<가게>-back.png` — 뒷모습. 화면 위로 걸어갈 때\n' +
          '**정면은 안 그린다**(서 있을 땐 마지막 방향 그대로 멈춘다 — 유저 결정).\n' +
          '선택: `<가게>-sleep.png`(조는 모습 — 없으면 코드가 옆모습에 💤를 얹는다),\n' +
          '승급판 `<가게>-side-1.png`(2등급)·`-2`(3등급) 등 — 없으면 1등급 것을 쓴다.\n' +
          '발끝은 아래 변에, 몸은 가로 정중앙에. 그림자·움직임 프레임 금지(코드 몫).\n' +
          '옛 make/sell 20장은 새 그림이 올 때까지만 돈다 — 추가 제작 금지.',
    rows: SHOPS.flatMap((sh) => [
      { id: `${sh.id}-side`, why: `${sh.name} 점장 — 옆(일·계산·서기 전부)` },
      { id: `${sh.id}-back`, why: `${sh.name} 점장 — 뒷모습(위로 걸을 때)` },
    ]) },
];
