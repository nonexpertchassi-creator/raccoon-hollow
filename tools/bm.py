...
# 서버비를 80만원으로 잡았다가 62배 부풀린 것을 알았다.
# 저장을 기기에 두고 클라우드는 백업으로만 쓰면 파이어베이스 무료 칸에서 끝난다 —
# 하루 6,000명이어도 한 달 5천원쯤이다. 진짜 나가는 건 스토어 계정값뿐.
SERVER_M = arg("--server", 13_000)         # 월 서버·도구 값 (계정 · 도메인)
ETC_M    = arg("--etc", 150_000)           # 월 잡비(그림 도구 · 법무 · 세무)
...
print("\n\n── 사람이 적으면 얼마를 버나 (거꾸로 말고 그냥) ──")
print(f"  한 사람 한 달 {won(arpu*(1-FEE))} (수수료 뗀 것)")
for dau in (100, 500, 1_000, 3_000, 6_000):
    mau = dau/DAU_MAU
    rev = mau*arpu*(1-FEE)
    print(f"  하루 {dau:>6,}명 → 한 달 {mau:>8,.0f}명 → 한 달 {won(rev):>12}"
          f" · 한 해 {won(rev*12):>14}")
print(f"\n  나가는 돈이 한 해 {won(cost_year)}이니")
print(f"  하루 {cost_year/12/(arpu*(1-FEE))*DAU_MAU:,.0f}명이 본전이다")
