...
const PARTS = [
  {
    ...
    open: [
      '**★ 이 게임에 끝이 없다.** `ending` 쪽지는 코드 어디서도 안 불린다. 엔딩 조건이 `STORY.md`에 글로만 있다',
      '**막이 끝났다는 표시가 없다.** 구역을 열면 소식에 한 줄 뜨는 게 전부다 — 잔치도, 돌아볼 것도 없다',
      '**끝 다음에 무엇이 있나** — 자유 영업? 새 마을? 계절? (2026-08-29 유저: *"이 게임의 끝을 정하는 순간 어렵다"* — 보류 중)',
